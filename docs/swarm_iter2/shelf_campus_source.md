...
Official scripts for visualization and evaluation (from [longcw/crossview_3d_pose_tracking](https://github.com/longcw/crossview_3d_pose_tracking)):
...
### 7.2 VoxelPose repo
- Provides pre-processed calibration files for Shelf/Campus.
- Provides 2D pose predictions (`pred_shelf_maskrcnn_hrnet_coco.pkl`, `pred_campus_maskrcnn_hrnet_coco.pkl`) in the repository itself.
- Source: [microsoft/voxelpose-pytorch](https://github.com/microsoft/voxelpose-pytorch)

### 7.3 Microsoft Graph-based Multi-view 3D Human Pose Estimation
This paper also reports results on Shelf/Campus.  Search GitHub for community re-implementations; the original Microsoft repository may have moved or been renamed.  If the exact repo URL is needed, search for the paper title on GitHub / PapersWithCode.

---

## 8. If Direct Links Become Unavailable

1. **Use the Google Drive mirror first** (§2.1).  If that disappears:
2. **Contact the authors of the Cross-View Tracking paper** (CVPR 2020):
   - Long Chen et al.
   - Repository: https://github.com/longcw/crossview_3d_pose_tracking/issues
3. **Check the VoxelPose repository** for the pre-processed calibration files and detection pickles; it may still host everything except raw frames.
4. **Academic request:** the original Campus data is hosted by EPFL; the original Shelf data was hosted by TUM.  If both fail, contacting the original publishers (CVLab EPFL for Campus, TUM Chair of Computer Aided Medical Procedures for Shelf) or the authors of papers using the data is the standard fallback.

---

## 9. Quick Summary

- **Best direct link:** Google Drive folder https://drive.google.com/drive/folders/1LJGcP2v0aQDmetnCzO2PiRP1v4jU6sFC?usp=drive_link (Campus + Shelf + extras)
- **Calibration:** JSON per camera with `R`, `T`, `fx/fy`, `cx/cy`, distortion `k/p`
- **GT 3D:** `annotation_3d.json`, list of `{timestamp, poses[{id, points_3d, scores}]}`
- **GT 2D:** `annotation_2d.json`, dict keyed by image path
- **Reference loaders:** `crossview_dataset.calib.calibration.Calibration.from_json()` and plain NumPy/OpenCV
- **No large files downloaded in this note.**
