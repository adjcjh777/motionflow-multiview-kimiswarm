...
We introduced a residual refinement head on top of temporal ray-attention fusion for calibrated multi-view 3D pose estimation. The method is simple, lightweight, and empirically strong: 10.46 mm on MPI-INF-3DHP and 0.62 mm on Human3.6M (CamPE+GraphJR). Integrated as a `MultiViewFusionPlugin` inside MotionFlow, it outputs a `HumanMotionIR` that supports quality gating and robot-profile-based retargeting. These properties make it a promising candidate for ICRA / CVPR 2027.
