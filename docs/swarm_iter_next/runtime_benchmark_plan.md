...
> Status: local WSL smoke numbers refreshed on 2026-08-12 12:49 UTC. Plan only; no A800 or training jobs launched.
...
|--------|---------|--------|
| `experiments/benchmark_runtime.py` | Single-frame latency, clip latency, peak memory, batch throughput for three model variants | `outputs/runtime_benchmark_20260812_124934.json`, `docs/swarm_iter_next/runtime_benchmark_report.md` |
...
Results (`outputs/runtime_benchmark_20260812_124934.json`):

| Model | Params | Single-frame (ms) | Clip (ms) | Batch throughput (fps) | Peak mem (MB) | 60 FPS? | 30 FPS? |
|-------|--------:|------------------:|----------:|------------------------:|--------------:|--------:|--------:|
| `RayAttentionFusionModelV3` | 134,497 | 86.98 | 86.98 | 45.02 | 10.65 | No | No |
| `RayAttentionFusionModelTemporal` | 217,825 | 88.53 | 86.61 | 575.15 | 24.90 | No | No |
| `RayAttentionFusionModelTemporalResidual` | 243,428 | 87.20 | 83.57 | 596.17 | 25.00 | No | No |
...
| 4 | 91.63 | 43.66 | 400 |
| 8 | 105.95 | 75.50 | 800 |
| 16 | 100.44 | 159.31 | 1600 |

CPU (B=1): 74.17 ms latency, 13.48 clips/s.
...
Our fresh local run (RTX 4090, torch 2.7.1+cu118) shows slightly worse latency/throughput than the paper draft table, likely due to a different PyTorch/CUDA version, background load, and the lack of `torch.compile`/TensorRT. The headline claim should be re-verified on the same target hardware and software stack before submission.
