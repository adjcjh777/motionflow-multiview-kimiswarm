...
- **v87 sparse-view residual head implemented and smoke-tested locally.** New module `motionflow_mv/fusion/sparse_view_residual_head_v87.py` wired into `MultiViewGeometryFusionV25`. 2-epoch smoke: val MPJPE **84.49 mm** → **121.01 mm** (small-sample smoke; no NaN/crash). A800 medium script to be created once v81 chain finishes.
- **Cross-domain H36M + AIST++ smoke with v86 separate sparse-view head completed locally.** 2 epochs on true-GT v2 mixed manifest: overall **101.35 mm**, per-domain H36M **100.71 mm**, AIST **0.64 mm**. AIST val is suspiciously low (likely DLT-fallback strong on full 9-view AIST); needs DLT baseline comparison.
...
- **Uncommitted changes:** `configs/sota_baselines/voxelpose_h36m_true_gt_v2.yaml`, `experiments/train_omniview_fusion_v5_webbridge_multi.py`, `motionflow_mv/fusion/multiview_geometry_fusion_v25.py`, `motionflow_mv/fusion/omniview_fusion_v5.py`, `scripts/run_voxelpose_true_gt_v2_a800.sh`, plus new files for v83/v87 smoke, v2 variable-view scripts, and three-dataset manifest.
