...
- AIST++-only medium fast v2 (`aistpp_only_medium_a800_fast_v2`) **relaunched on A800 GPU 5** with clean canonical `.npz` after the NaN fix, but crashed in the mixed-loader collate function because two sequences became empty after dropping NaN 2D frames. Converter now zeroes NaN 2D instead of dropping frames; canonical `.npz` is regenerating locally and will be resynced to A800 before relaunching on GPU 5.
