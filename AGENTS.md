...
| v53 PSC smoke | v45/v46/v50/v51/v52 + physical-space calibration (floor + bone + gated residual) | **Fixed NaN root cause** (`sqrt(weights).clamp` -> `clamp().sqrt()`); also clamped canonical `log_bone_lengths` in v53 to prevent unbounded exponential growth; A800 tiny smoke best 98.65 mm; local tiny smoke best 78.76 mm; v2 medium diverged (train loss >11); v3 medium (same recipe + clamp fix) train loss converged to ~6.45 but stalled during first validation; see issue #183 |
| v57 DC-PSC smoke | v45/v46/v50/v51/v52 + domain-conditional physical-space calibration (FiLM on floor/bone/residual, per-domain gate) | Local smoke done; epoch-2 val_MPJPE 102.38 mm (baseline v52 78.68 mm; tiny smoke, may overfit due to extra ~42k domain-conditional params); A800 full run queued in `launch_v33_a800_queue.py`; fallback v58 SD-PSC implemented and smoking; see issue #186 |
| v58 SD-PSC smoke | v45/v46/v50/v51/v52 + simplified domain-conditional PSC (per-domain bone lengths + floor offset, shared residual MLP) | Local smoke running; removes ~42k v57 domain-FiLM params; see issue #186 |
