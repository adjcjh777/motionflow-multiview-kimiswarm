...
| v36 uncertainty-gated iterative graph refinement | v35 TVJGN + per-node uncertainty gating + iterative refinement; self-evolving fusion | merged | Smoke passed (clip_len=3, d=32, 10 samples): val_MPJPE 100.47mm; PR #145 merged; A800 queue includes on-v34-vjgn, on-v35-tvjgn, on-v34-hmsp-geometry-vjgn, and n_iters=1 ablation; full local run in progress (epoch 1) |
| v37 self-critique view reliability | v36 UGIGR + per-(view,joint) reliability score learned from reprojection residuals; self-evolution | merged | Module and wiring merged; smoke queued after local GPU frees; A800 queue entry added via launch_v33_a800_queue.py |
