"""Cross-view residual model with per-view uncertainty (log-variance) fusion.

Extends the principal-point corrected cross-view residual model by replacing
the sigmoid per-view weight with a predicted log-variance.  The DLT weight is

    weight = confidence * exp(-log_var)

so views with higher predicted variance receive lower weight.  An optional
reprojection negative-log-likelihood loss can supervise the uncertainties.
"""
...
class CrossviewResidualUncertaintyModel(RayAttentionFusionModelTemporalCrossviewResidualPrincipalPoint):
    """Principal-point cross-view residual model with uncertainty-weighted DLT.
    ...
    def forward(
        ...
        # Per-frame features (uses corrected intrinsics).
        ...
        # Predict log-variance per (view, joint).
        ...
        weights = precision * confidences * visibility  # (B*T, V, J)
