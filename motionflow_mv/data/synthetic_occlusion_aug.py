...
This better mimics real occlusions: a limb or the torso tends to disappear as a
whole when it is behind another body part or outside the image border.
...
# Maps supported skeleton aliases to group tables.
_SKELETON_GROUPS = {
...
def occlude_joint_groups(
    ...
    if isinstance(group_indices, str):
        if group_indices not in _SKELETON_GROUPS:
            raise ValueError(
                f"Unsupported skeleton alias: {group_indices!r}. "
                f"Supported: {list(_SKELETON_GROUPS.keys())}"
            )
...
def random_occlude_joint_groups(
    ...
    if isinstance(skeleton, str):
        if skeleton not in _SKELETON_GROUPS:
            raise ValueError(
                f"Unsupported skeleton alias: {skeleton!r}. "
                f"Supported: {list(_SKELETON_GROUPS.keys())}"
            )
...
class SyntheticJointOcclusionAugmenter:
    ...
    Args:
        ...
        temporal_consistency: If ``True`` and the input has shape
            ``(B, T, V, J, C)``, the same group occlusion mask is applied across
            the temporal dimension ``T`` for each sample.  This simulates
            occlusions that persist over short clips.
    ...
    def __init__(
        ...
        if skeleton not in _SKELETON_GROUPS:
            raise ValueError(
                f"Unsupported skeleton: {skeleton!r}. Supported: {list(_SKELETON_GROUPS.keys())}"
            )
    ...
    def _apply_temporal(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for g_idx, joint_indices in enumerate(group_list):
            ...
            if joint_indices_t.numel() == 0:
                continue
            active = mask[:, g_idx]  # (B,)
            # Expand to (B, 1, 1, len(joints), 1) and apply to all T frames + V views.
