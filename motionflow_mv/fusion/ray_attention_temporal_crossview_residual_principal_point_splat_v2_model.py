"""Gaussian-splatting pose regularizer with view-dependent covariance.

Extends ``RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointSplat``
with an additional per-view covariance head.  A shared world-space Gaussian
remains the backbone, but each view/joint can scale the projected covariance
through a residual log-standard-deviation head computed from per-view
spatio-temporal features.  This lets the model express view-specific
uncertainty (occlusion, motion blur, calibration residual) without giving up
the geometric interpretability of the 3-D Gaussian.
"""

import torch
import torch.nn as nn

from .ray_attention_temporal_crossview_residual_principal_point_splat_model import (
    RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointSplat,
)


class RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointSplatV2(
    RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointSplat,
):
    """PP-residual splat model with per-view residual covariance.

    Parameters
    ----------
    return_view_covariance:
        If ``True`` (default), the forward pass also returns a per-view
        residual log-std tensor of shape ``(B, T, V, J, 3)`` in addition to
        the world log-std ``(B, T, J, 3)``.
    See ``RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointSplat``
    for the remaining args.
    """

    def __init__(self, *args, return_view_covariance: bool = True, **kwargs):
        super().__init__(*args, **kwargs)
        self.return_view_covariance = return_view_covariance
        # Per-view residual covariance head: maps per-view features to a
        # residual log-std for each axis, per view, per joint.
        self.view_covariance_head = nn.Sequential(
            nn.Linear(self.d, self.residual_hidden),
            nn.ReLU(),
    ...
    def forward(self, x, cameras=None, K=None, R=None, t=None):
        ...
        # World-space per-joint 3-D Gaussian log std.
        log_std_world = self.covariance_head(residual_input)  # (B*T, J, 3)
        log_std_world = log_std_world.view(B, T, J, 3)

        # Per-view residual log std (world axes, diagonal covariance).
        log_std_view = self.view_covariance_head(feat)  # (B*T, V, J, 3)
        log_std_view = log_std_view.view(B, T, V, J, 3)
        ...
        if squeeze_output:
            ...
            log_std_world = log_std_world.squeeze(1)
            log_std_view = log_std_view.squeeze(1)
            if self.return_visibility:
        ...
        if self.return_pp_delta:
            ...
            if self.return_covariance:
                out.append(log_std_world)
            if self.return_view_covariance:
                out.append(log_std_view)
            return tuple(out)
        if self.return_visibility:
        ...
        if self.return_covariance:
            return pred_3d, weights, log_std_world
        if self.return_view_covariance:
            return pred_3d, weights, log_std_world, log_std_view
