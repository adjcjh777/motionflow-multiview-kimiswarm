"""Lightweight epipolar-geometry bias: late-layer injection only.

This is a lighter sibling of the full EpipolarBiasV2 model.  Instead of
injecting an epipolar relative-position bias into *every* spatio-temporal
transformer layer, the bias is applied only to the *last* ST layer.  All
earlier layers remain standard ``nn.TransformerEncoderLayer`` layers.
"""

import torch
import torch.nn as nn

from .ray_attention_temporal_crossview_residual_principal_point_epipolar_bias_v2_model import (
...
class RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointEpipolarBiasV2Lite(
    ...
    """Cross-view temporal residual model with epipolar bias on the last ST layer only."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Restore standard TransformerEncoderLayer for all but the last ST layer.
        if len(self.st_transformer) > 1:
            for idx in range(len(self.st_transformer) - 1):
                self.st_transformer[idx] = nn.TransformerEncoderLayer(
                    d_model=self.d,
                    nhead=self.n_heads,
    ...
    def forward(self, x, cameras=None, K=None, R=None, t=None):
        ...
        for idx, layer in enumerate(self.st_transformer):
            if idx == n_layers - 1:
