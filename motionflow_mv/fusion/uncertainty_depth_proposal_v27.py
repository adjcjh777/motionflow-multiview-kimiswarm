...
learned per-ray Gaussian depth distribution.  The head predicts a depth mean
and log-standard-deviation for each (view, joint) ray, samples a small number
of depth hypotheses continuously, and scores them with an MLP that also sees
the predicted uncertainty.  At inference the mean depth is used directly, so
the head is deterministic and fast.

This is intentionally the *single-Gaussian* minimal variant.  Mixture models
...
class UncertaintyDepthProposalTriangulation(nn.Module):
    ...
    n_ray_samples:
        ...
        At inference this is ignored and the mean depth is used.
    init_z_min, init_z_max:
        Initial depth range.  ``mu`` is initialised near the midpoint.
    min_sigma:
    ...
    def __init__(
        ...
        # Initialise mu near the midpoint and log_sigma to a large value.
        self.dist_mlp[-1].bias.data[0] = (init_z_min + init_z_max) / 2.0
        self.dist_mlp[-1].bias.data[1] = 0.0  # sigma = 1, will be widened below.
    ...
    def forward(
        ...
        dist_out = self.dist_mlp(dist_feat)  # (B, T, V, J, 2)
        mu = dist_out[..., 0]  # (B, T, V, J)
        log_sigma = dist_out[..., 1]
        sigma = torch.exp(log_sigma).clamp(min=self.min_sigma)

        # Sample depths: during training reparameterised, at inference use mean.
        if self.training:
            # Standard Normal samples; shared across batch/time to keep memory low.
            eps = torch.randn(B, T, V, J, self.n_ray_samples, device=direction.device, dtype=direction.dtype)
            # Clamp sigma to a reasonable range for stability.
            sigma_sample = sigma[..., None].clamp(max=2.0)
            z_vals = mu[..., None] + sigma_sample * eps  # (B, T, V, J, S)
        else:
            z_vals = mu[..., None].expand(-1, -1, -1, -1, self.n_ray_samples)
        ...
        sigma_exp = sigma[..., None].expand(-1, -1, -1, -1, self.n_ray_samples)  # (B, T, V, J, S)
        ...
        # Uncertainty regularisation: keep sigma close to a target, not too small/large.
        sigma_target = 0.2
        uncertainty_loss = self.uncertainty_loss_weight * (sigma - sigma_target).abs().mean()
