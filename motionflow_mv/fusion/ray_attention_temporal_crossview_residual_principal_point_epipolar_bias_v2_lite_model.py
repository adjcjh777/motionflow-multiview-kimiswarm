...
class RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointEpipolarBiasV2Lite(
    ...
    """Apply epipolar attention bias only in the last ST transformer layer."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(self.st_transformer) > 0:
            self.st_transformer[-1] = EpipolarBiasedTransformerEncoderLayer(
                d_model=self.d,
                nhead=self.n_heads,
    ...
    def forward(self, x, cameras=None, K=None, R=None, t=None):
        ...
        with torch.no_grad():
            per_frame_bias = compute_per_frame_epipolar_bias(
                ...
                t,
                points_2d,
                temperature=self.epipolar_temperature,
            ).view(B, T, V, V)
        attn_bias = build_temporal_bias_from_frames(
            per_frame_bias,
        ...
        weights = (weights * confidences * visibility).clamp(min=1e-4)
        ...
        pred_3d = pred_3d_raw + self.residual_mlp(residual_input)
