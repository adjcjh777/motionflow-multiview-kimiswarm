"""v58 Simplified Domain-Conditional Physical-Space Calibration (SD-PSC).

A lightweight variant of v53 Physical-Space Calibration with only two
domain-conditional parameters:

* ``canonical_log_bone_lengths`` – per-domain canonical bone lengths
  (already per-domain in v53, retained here).
* ``floor_domain_offset`` – a per-domain additive floor-height offset.

Everything else (floor MLP, bone MLP, residual MLP and the residual gate)
is shared across domains, making the module much smaller than the full v57
DC-PSC while still allowing domains such as WebBridge, H36M, MPI and 3DPW
to adapt their floor plane and canonical bone lengths.

The module is identity at initialization: the final layers of the floor,
bone and residual MLPs are zero-initialized, the residual gate is initialized
to a very small value (``residual_gate_init=-6.0`` => ``sigmoid(gate) ≈ 0.0025``),
and ``floor_domain_offset`` is zero, so the output pose equals the input pose
...
class SimplifiedDomainPhysicalCalibrationV58(nn.Module):
    """Simplified domain-conditional physical-space calibration head.
    ...
    num_domains:
        Maximum number of domains for per-domain canonical bone lengths and
        the per-domain floor offset.
    use_floor:
    ...
    def __init__(
        ...
        # Per-domain floor height offset. Added to the shared MLP's predicted
        # floor height, zero at init so the module is identity.
        self.floor_domain_offset = nn.Parameter(torch.zeros(num_domains))
        ...
        # Gated physical residual MLP (shared across domains).
        ...
        # Shared scalar residual gate (unlike v57 per-domain gate).
    ...
    def _floor_term(
        ...
        # Add the per-domain floor height offset.
        if domain_id is not None and domain_id.numel() > 0:
            d = domain_id.long().clamp(0, self.floor_domain_offset.shape[0] - 1)
            domain_floor_offset = self.floor_domain_offset[d]  # (B,)
        else:
            domain_floor_offset = torch.zeros(B, device=X.device, dtype=X.dtype)

        floor_height = foot_heights.min(dim=-1).values + floor_offset + domain_floor_offset.unsqueeze(-1)
    ...
    def _bone_scale_term(
        ...
        """Canonical bone-length term with per-domain canonical lengths.
    ...
    def forward(
        ...
        if self.use_floor:
            floor_loss, floor_translation = self._floor_term(pred_3d_uwt, uwt_weights, view_mask, domain_id)
