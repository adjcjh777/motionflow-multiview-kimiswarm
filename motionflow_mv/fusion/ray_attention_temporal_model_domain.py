"""Temporal ray-aware attention fusion with a camera/domain-adaptation MLP.

This extends :class:`RayAttentionFusionModelTemporal` by adding a lightweight MLP
that consumes the per-view camera intrinsics/extrinsics and predicts a
"domain" embedding.  The embedding is added to the per-view features and allows
the model to adapt to mixed camera rigs within a single batch.

The rest of the architecture (per-frame v3 encoder, temporal transformer,
weighted DLT triangulation) is unchanged.
"""
...
class RayAttentionFusionModelTemporalDomain(nn.Module):
    """Temporal ray-aware fusion with a camera-intrinsics/extrinsics domain MLP.
    ...
    def __init__(
        ...
        # Domain-adaptation MLP: maps per-view camera parameters to a domain vector.
        self.camera_domain_mlp = nn.Sequential(
            nn.Linear(9 + 9 + 3, d),
            nn.ReLU(),
            nn.Linear(d, d),
        )
    ...
    def _extract_frame_features(
        ...
        # Camera embedding + domain embedding.
        ...
        domain_emb = self.camera_domain_mlp(camera_feat)  # (N, V, d)
        domain_emb = domain_emb[:, :, None, :].expand(N, V, J, self.d)
        feat = feat + camera_emb + domain_emb
    ...
    def forward(
        ...
        # Clamp to avoid a degenerate DLT least-squares problem when a joint has
        # zero or near-zero weight across all views (can happen with heavy
        # dropout augmentation or extreme outlier views in mixed-rig batches).
        tri_weights = weights.clamp(min=1e-4)

        Rt = torch.cat([R, t[..., None]], dim=-1)  # (B*T, V, 3, 4)
        P = K @ Rt
        pred_3d = _triangulate_weighted_dlt(points_2d, tri_weights, P)  # (B*T, J, 3)
