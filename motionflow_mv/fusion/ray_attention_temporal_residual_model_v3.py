"""Temporal ray-aware attention fusion with residual refinement (v3).

This is a thin wrapper around ``RayAttentionFusionModelTemporalResidual`` that
also exposes the *raw* DLT triangulated pose before the residual correction is
applied.  It is useful for diagnostic visualizations that need to compare the
raw estimate, the refined estimate, and ground truth.
"""
...
class RayAttentionFusionModelTemporalResidualV3(RayAttentionFusionModelTemporalResidual):
    """Same as ``RayAttentionFusionModelTemporalResidual`` but can return raw DLT pose.
    ...
    Notes
    -----
    ``forward`` accepts an extra boolean flag ``return_raw``.  When ``True``,
    the returned tuple is ``(pred_3d, weights, pred_3d_raw)`` instead of the
    usual ``(pred_3d, weights)``.
    """
    ...
    def forward(
        ...
        pred_3d_raw = pred_3d_raw.view(B, T, J, 3)
        weights = weights.view(B, T, V, J)

        if squeeze_output:
            pred_3d = pred_3d.squeeze(1)
            pred_3d_raw = pred_3d_raw.squeeze(1)
            weights = weights.squeeze(1)

        if return_raw:
            return pred_3d, weights, pred_3d_raw
