"""Spatio-temporal ray-aware attention fusion.

Extends the temporal model by replacing the per-(view,joint) temporal transformer
with a single transformer that attends jointly over time **and** views for every
joint.  For each joint, the input is a grid of (time, view) tokens; the same
self-attention operation therefore mixes temporal and cross-view information in
one shot.  This keeps the architecture minimal while giving temporal tokens
access to all views at every time step.

Input / output semantics are identical to `RayAttentionFusionModelTemporal`.
"""
...
class RayAttentionFusionModelTemporalCrossview(nn.Module):
    """Ray-aware attention fusion with joint temporal + cross-view attention.
    ...
    def __init__(
        ...
        # Spatio-temporal positional embeddings for time and view.
        ...
        # Joint spatio-temporal transformer (time + views).
    ...
    def forward(
        ...
        # Reshape to (B, T, V, J, d) for spatio-temporal attention.
        feat = feat.view(B, T, V, J, self.d)
        # Add time and view positional embeddings.
        ...
        # For each joint, attend over the (time, view) grid.
        # (B, J, T, V, d) -> (B*J, T*V, d)
        ...
        # Guard against all-zero weights that make DLT lstsq singular.
