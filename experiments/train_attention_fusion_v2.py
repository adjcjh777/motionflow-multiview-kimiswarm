"""Train geometry-aware AttentionFusionV2 on Shelf/Campus/VoxelPose data.

This script converts matched 2D predictions into training samples for the
AttentionFusionModelV2, which also receives flattened camera projection
matrices as input.

Usage:
    /d/anaconda3/envs/jz_py310/python.exe experiments/train_attention_fusion_v2.py \
...
def build_dataset(loader, frame_start: int, frame_end: int):
    """Return (inputs, cameras, targets) tensors for matched frames.

    inputs:  (N, V, J, 3)  -> (x, y, confidence)
    cameras: (N, V, 12)    -> flattened projection matrices
    ...
    proj_flat = proj_matrices.reshape(-1, 12)  # (V, 12)
    pipeline = MultiViewPipeline(estimator=None)

    inputs, camera_inputs, targets = [], [], []
    for frame_idx in range(frame_start, frame_end + 1):
        ...
        camera_inputs.append(torch.tensor(proj_flat, dtype=torch.float32))
    ...
    return torch.stack(inputs), torch.stack(camera_inputs), torch.stack(targets)
...
def main():
    ...
    inputs, cameras, targets = build_dataset(loader, args.frame_start, args.frame_end)
    ...
    train_cameras, val_cameras = cameras[n_val:], cameras[:n_val]
    train_targets, val_targets = targets[n_val:], targets[:n_val]

    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(train_inputs, train_cameras, train_targets),
        batch_size=args.batch_size,
        shuffle=True,
    )
    val_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(val_inputs, val_cameras, val_targets),
        batch_size=args.batch_size,
    )
    ...
    output_name = f"attention_fusion_v2_{args.dataset}.pth"
