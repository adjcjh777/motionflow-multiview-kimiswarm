"""Train the camera-domain temporal model on MPI-INF-3DHP clips.
...
-----
    conda run -n mf python experiments/train_ray_attention_temporal_mpiinf3dhp_domain.py \
        ...
        --clip_len 13 --epochs 3 --train_samples 1000

Differences from the baseline temporal script:
* Uses ``RayAttentionFusionModelTemporalDomain``, which adds a camera domain MLP.
* Each batch may contain clips drawn from multiple sequences, therefore mixed
  camera rigs (different R/t per sample).  Per-sample ``K, R, t`` are passed
  through to the model so the domain MLP can adapt.
...
from motionflow_mv.fusion.ray_attention_temporal_model_domain import RayAttentionFusionModelTemporalDomain
...
def main():
    parser = argparse.ArgumentParser(description="Train camera-domain temporal ray-attention fusion on MPI-INF-3DHP")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_mpiinf3dhp_domain.pth")
    ...
    model = RayAttentionFusionModelTemporalDomain(
