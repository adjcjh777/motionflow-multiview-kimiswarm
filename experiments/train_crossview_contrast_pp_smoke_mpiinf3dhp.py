"""Smoke trainer for cross-view contrastive pose representation (Tier-1 iter15 proposal).

This is a thin wrapper around ``train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py``
that runs a 5-epoch smoke on a small MPI-INF-3DHP split with the cross-view
contrastive model and its auxiliary InfoNCE loss.
"""
...
def main():
    ...
    cmd = [
        ...
        "--cam_aug_pp", "5.0",
        "--cam_aug_focal", "0.01",
        "--output", "outputs/crossview_contrast_pp_smoke.pth",
    ]
