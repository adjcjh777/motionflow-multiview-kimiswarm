"""Smoke test for the reprojection-gated residual head trainer.

Generates a tiny synthetic multi-view .npz dataset, then runs
``experiments/train_ray_attention_temporal_residual_mpiinf3dhp.py`` with
``--use_reproj_gate`` for a single epoch on the CPU.  This is intended only as a
quick integration test; it does not produce a meaningful model.

Usage
-----
    python experiments/smoke_reprojgate.py
"""
...
def _random_skeleton(j: int, rng: np.random.Generator) -> np.ndarray:
    # Generate a simple tree-like skeleton with plausible limb lengths.
    X = rng.normal(0.0, 0.15, size=(j, 3))
    X[:, 2] += 2.0  # stand in front of cameras
    return X
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="reprojgate_smoke_") as tmp:
        tmp = Path(tmp)
        ...
        cmd = [
            ...
            "--train_samples", "20",
            "--val_stride", "1",
            "--output", str(tmp / "reprojgate_smoke.pth"),
        ]
