"""Train RayAttentionFusionModelTemporalCrossview on MPI-INF-3DHP clips.

This is a short-run smoke-test script for the combined temporal+cross-view
model.  It reuses the same dataset helpers as the temporal baseline, but runs
fewer epochs and samples by default.
...
-------
    conda run -n mf python experiments/train_ray_attention_temporal_crossview_mpiinf3dhp_v1.py \
        ...
        --clip_len 13 --epochs 3 --batch_size 4
"""
...
from motionflow_mv.fusion.ray_attention_temporal_crossview_model import RayAttentionFusionModelTemporalCrossview
    ...
    with torch.no_grad():
        for batch_idx, (xb, yb, K, R, t) in enumerate(loader):
            ...
            if not torch.isfinite(pred).all():
                print(f"WARNING: non-finite prediction at val batch {batch_idx}; pred nan={torch.isnan(pred).any().item()}, inf={torch.isinf(pred).any().item()}; weights nan={torch.isnan(weights).any().item()}")
                continue
...
def main():
    parser = argparse.ArgumentParser(description="Train temporal+cross-view ray-attention fusion on MPI-INF-3DHP")
    ...
    parser.add_argument("--train_samples", type=int, default=1000, help="Random clips per train sequence")
    parser.add_argument("--augment", action="store_true", help="Enable per-clip augmentation (noise/dropout/outliers)")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_crossview_mpiinf3dhp.pth")
    ...
    model = RayAttentionFusionModelTemporalCrossview(
