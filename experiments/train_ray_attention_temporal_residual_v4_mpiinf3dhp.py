"""Train deep (N-block) residual RayAttentionFusionModel on MPI-INF-3DHP clips.

Extends ``train_ray_attention_temporal_residual_mpiinf3dhp.py`` to support a
stacked residual refinement head.  Use ``--n_residual_blocks`` to choose 1
(single-block, equivalent to v2) or 3 (deep, v4).
...
-----
    conda run -n mf python experiments/train_ray_attention_temporal_residual_v4_mpiinf3dhp.py \
        ...
        --n_residual_blocks 3 --clip_len 13 --epochs 10

If the MPI-INF-3DHP files are not present, the same script can be run on any
.npz with the keys ``points_2d``, ``confidences``, ``joints_3d``, ``camera_K``,
``camera_R``, ``camera_t`` (e.g. H36M canonical .npz) as a smoke test.
"""
...
from motionflow_mv.fusion.ray_attention_temporal_residual_v4_model import (
    RayAttentionFusionModelTemporalResidualV4,
)
...
def main():
    parser = argparse.ArgumentParser(description="Train temporal ray-attention fusion with deep residual refinement")
    ...
    parser.add_argument("--n_residual_blocks", type=int, default=3, help="Number of stacked residual blocks")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_residual_v4_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, "
          f"residual_hidden={args.residual_hidden}, n_residual_blocks={args.n_residual_blocks}")

    model = RayAttentionFusionModelTemporalResidualV4(
        j=j, d=args.d, n_views=n_views, n_temporal_layers=args.n_temporal_layers,
        residual_hidden=args.residual_hidden, n_residual_blocks=args.n_residual_blocks,
    ).to(device)
