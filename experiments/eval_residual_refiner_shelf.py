"""Evaluate the trained ResidualRefinerModel on Shelf and compare to DLT.

Usage:
    /d/anaconda3/envs/jz_py310/python.exe experiments/eval_residual_refiner_shelf.py
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ResidualRefinerModel on Shelf.")
    parser.add_argument("--checkpoint", type=str, default="outputs/residual_refiner_shelf.pth")
    ...
    model = ResidualRefinerModel(j=17, d=args.d, n_views=5).to(device)
    ...
    with torch.no_grad():
        for fid, item in dataset.items():
            ...
            pred = model(inp_t, baseline).squeeze(0).cpu().numpy()
    ...
    print(f"Residual| {model_errors.mean():.2f} | {np.median(model_errors):.2f} | {model_errors.max():.2f}")
