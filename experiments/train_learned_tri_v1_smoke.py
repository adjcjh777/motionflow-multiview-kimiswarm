"""Smoke training for the learned Gauss-Newton triangulation v1 head.

Runs a minimal training loop for
``RayAttentionFusionModelTemporalResidualLearnedTri`` on a tiny synthetic
multi-view dataset.  The goal is to verify that the model can be instantiated,
its GN triangulation head can be differentiated, and a few epochs of training
complete without errors on CPU (or CUDA if available).

Usage
-----
    conda run -n mf python experiments/train_learned_tri_v1_smoke.py
    conda run -n mf python experiments/train_learned_tri_v1_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/learned_tri_v1_smoke.pth``
"""
...
class SyntheticSmokeDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # Generate smooth random 3D poses.
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for learned Gauss-Newton triangulation v1")
    ...
    parser.add_argument("--n_temporal_layers", type=int, default=1, help="Temporal transformer layers")
    ...
    parser.add_argument("--output", type=str, default="outputs/learned_tri_v1_smoke.pth")
    ...
    model = RayAttentionFusionModelTemporalResidualLearnedTri(
        j=n_joints,
        d=args.d,
        n_views=4,
    ...
    print(
        ...
        f"gn_iters={args.gn_iters}, params={n_params}"
    )
