"""Train OmniMultiViewFusionV2 on one or more WebBridge datasets.

This script reads split manifests (YAML files with ``train``/``val``/``test`` keys)
and trains ``OmniMultiViewFusionV2`` on the concatenated data.  It keeps the same
loss mix and hyperparameters as ``train_omniview_fusion_v2_mpiinf3dhp.py`` and adds
...
from motionflow_mv.fusion.omniview_fusion_v2 import OmniMultiViewFusionV2  # noqa: E402
...
def uncertainty_nll_loss(
    ...
    Args:
        ...
        confidences: (B, T, V, J)
        K, R, t: (B, V, 3, 3) / (B, V, 3)
        L: (B, T, V, J, 2, 2) Cholesky factor of 2-D covariance.

    Returns:
        Scalar NLL averaged over valid observations.
    """
    ...
    # Solve L y = r, then L^T a = y  => a = Sigma^{-1} r
    ...
    # log det(Sigma) = 2 * (log L00 + log L11)
...
class OmniMultiViewTrainer(TrainerV2):
    """TrainerV2 pre-wired with the OmniMultiViewFusionV2 loss mix."""
...
def _validate_dataset_consistency(
    ...
    OmniMultiViewFusionV2 is built for a single skeleton/view count, so mixed
    manifests must be split by skeleton before training.  Returns the common
    (n_views, n_joints).
    """
    ...
    unique = set(shapes)
    if len(unique) != 1:
        # Group by shape for a clear error message.
    ...
    if graph_num_layers > 0 and n_joints not in (17, 28):
        raise ValueError(
            f"OmniMultiViewFusionV2 graph attention only supports 17 or 28 joints, "
...
def build_datasets(args: Namespace) -> Tuple[torch.utils.data.Dataset, torch.utils.data.Dataset, int, int]:
    ...
    # If multiple val files, concatenate them as well.
    if len(val_files) > 1:
...
def parse_args() -> Namespace:
    parser = argparse.ArgumentParser(
        description="Train OmniMultiViewFusionV2 on one or more WebBridge datasets",
    )
    # Data
    parser.add_argument(
        "--manifest",
    ...
    # Warm-start
    parser.add_argument("--warm_start", type=str, default=None, help="Path to bayesian_tri_v2 checkpoint for warm-starting")
    ...
    # I/O
    parser.add_argument("--output", type=str, default="outputs/omniview_fusion_v2_webbridge_multi.pth", help="Checkpoint path")
    ...
    if args.smoke:
        # Smoke overrides for a fast, self-contained run.
        args.epochs = 1
        args.d = 32
...
def main():
    ...
    if n_joints != 17:
        ...
        # For other skeletons (e.g. 24-joint 3DPW) graph attention must be disabled
        # via --graph_num_layers 0, which is enforced by _validate_dataset_consistency.
