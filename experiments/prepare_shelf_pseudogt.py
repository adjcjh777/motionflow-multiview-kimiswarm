"""Build a pseudo-GT Shelf dataset from detection.json + result_3d.json.

The detection.json holds per-camera 2D keypoints (pixels) and the
result_3d.json holds triangulated 3D joints (cm). This script aligns them
by timestamp and produces an dataset compatible with
`train_ray_attention_v2_real.py`.

Output: data/shelf_campus/Shelf_Seq1/pseudogt.npz
"""
...
def main():
    root = Path("data/shelf_campus/Shelf_Seq1")
    ...
    for gt_frame in res3d:
        ...
        # Ensure all 5 cameras present.
        if not all(cam in det_at_ts for cam in camera_names):
            continue

        # For each camera, take the first detected person.
        p2d_list = []
        conf_list = []
        ...
        p2d = np.array(p2d_list, dtype=np.float64)  # (V, J, 2)
        conf = np.array(conf_list, dtype=np.float64)  # (V, J)

        # Build a consistent 3D target by triangulating the 2D detections.
        # result_3d coordinates are not aligned with these cameras, so we ignore
        # them and create a DLT-consistent pseudo-GT instead.
