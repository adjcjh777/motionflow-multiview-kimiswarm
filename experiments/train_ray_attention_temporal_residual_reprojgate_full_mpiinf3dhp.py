"""Full 30-epoch training wrapper for the reprojection-gated residual head.

Launches ``experiments/train_ray_attention_temporal_residual_mpiinf3dhp.py``
with the standard MPI-INF-3DHP cross-subject split and enables the
reprojection-error gate in the residual refinement head.  The gate lets the
residual correction scale adaptively based on the per-joint reprojection-error
summary (mean/std/max/inlier fraction), which should suppress wild residual
updates when the raw triangulation is already consistent across views.
...
Usage
-----
    conda run -n mf python experiments/train_ray_attention_temporal_residual_reprojgate_full_mpiinf3dhp.py
"""
...
def main():
    ...
    cmd = [
        ...
        *[str(p) for p in train_files],
        ...
        "--train_samples", "250",
        "--output",
        str(root / "outputs" / "ray_attention_temporal_residual_reprojgate_full.pth"),
    ]
