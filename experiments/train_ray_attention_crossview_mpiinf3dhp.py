"""Train RayAttentionFusionModelCrossView on MPI-INF-3DHP clips.
...
The script mirrors ``train_ray_attention_temporal_mpiinf3dhp.py`` but swaps the
model for the cross-view transformer variant.
"""
...
from motionflow_mv.fusion.ray_attention_crossview_model import RayAttentionFusionModelCrossView
...
def main():
    parser = argparse.ArgumentParser(description="Train cross-view ray-attention fusion on MPI-INF-3DHP")
    ...
    parser.add_argument("--d_cross", type=int, default=32)
    parser.add_argument("--n_crossview_layers", type=int, default=1)
    ...
    parser.add_argument("--val_max_clips", type=int, default=None, help="Limit validation clips (None = all)")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--output", type=str, default="outputs/ray_attention_crossview_mpiinf3dhp.pth")
    ...
    if args.val_max_clips is not None and len(val_dataset) > args.val_max_clips:
        val_dataset = torch.utils.data.Subset(val_dataset, range(args.val_max_clips))
    ...
    model = RayAttentionFusionModelCrossView(
        j=j,
        d=args.d,
        n_views=n_views,
        n_crossview_layers=args.n_crossview_layers,
