"""Train ray-aware attention fusion v2 on synthetic multi-view data.

Usage:
    /d/anaconda3/envs/jz_py310/python.exe experiments/train_ray_attention_v2_synthetic.py \
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val:
            ...
            torch.save(model.state_dict(), output_dir / f"ray_attention_v1_{dataset_stem}.pth")

        if epoch % 5 == 0 or epoch == 1:
            print(f"Epoch {epoch}: train_loss={train_loss:.4f}, val_loss={val_loss:.4f}, val_MPJPE={val_mpjpe:.4f}m")

    print(f"Best val_loss={best_val:.4f}, checkpoint: {output_dir / 'ray_attention_v1_synthetic.pth'}")
