"""Train RayAttentionFusionModelTemporalResidualCritic on MPI-INF-3DHP clips.
...
swapping in ``RayAttentionFusionModelTemporalResidualCritic``, which adds a
small self-critic temporal refiner on top of the residual-corrected trajectory.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train temporal ray-attention fusion with residual + critic refinement on MPI-INF-3DHP")
    ...
    parser.add_argument("--critic_layers", type=int, default=2, help="Temporal transformer layers in the critic")
    parser.add_argument("--critic_hidden", type=int, default=128, help="Hidden dimension of the critic pose embedding")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_residual_critic_mpiinf3dhp.pth")
    parser.add_argument("--resume", type=str, default=None, help="Checkpoint to warm-start the base residual weights (critic stays random)")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, "
          f"residual_hidden={args.residual_hidden}, critic_layers={args.critic_layers}, critic_hidden={args.critic_hidden}")

    model = RayAttentionFusionModelTemporalResidualCritic(
    ...
    if args.resume is not None:
        print(f"Loading base weights from {args.resume} (strict=False)")
