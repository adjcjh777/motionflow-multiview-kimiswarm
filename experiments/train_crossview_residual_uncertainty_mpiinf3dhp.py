"""Train CrossviewResidualUncertaintyModel on MPI-INF-3DHP.

Thin wrapper around ``train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py``
that swaps in ``CrossviewResidualUncertaintyModel``.  The model predicts per-view
log-variance and uses precision ``exp(-log_var)`` as the DLT weight, then adds
the auxiliary reprojection NLL loss to the MSE loss.
...
-----
    python experiments/train_crossview_residual_uncertainty_mpiinf3dhp.py \
        ...
        --uncertainty_loss_weight 0.1 --epochs 10
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train cross-view residual + principal-point + uncertainty model on MPI-INF-3DHP")
    ...
    parser.add_argument("--output", type=str, default="outputs/crossview_residual_uncertainty_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, n_st_layers={args.n_st_layers}, "
          ...
          f"uncertainty_loss_weight={args.uncertainty_loss_weight}, focal_max_scale={args.focal_max_scale}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, K, R, t in train_loader:
            ...
            if args.pp_loss_weight > 0.0:
                # The PP model returns pp_delta as the 3rd output; uncertainty model returns it as the 3rd too.
                # CrossviewResidualUncertaintyModel does NOT return pp_delta, so we skip PP supervision for now.
                pass
