"""Train visibility-gated cross-view residual v2 on MPI-INF-3DHP.

Wraps the principal-point training script and swaps in
``CrossviewResidualVisibilityV2``, adding a BCE visibility loss on the
learned per-view/per-joint visibility mask.
"""
...
# Reuse data loaders / augmentation from the PP trainer.
...
def main():
    parser = argparse.ArgumentParser(
        description="Train visibility-gated cross-view residual v2 on MPI-INF-3DHP"
    )
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_crossview_residual_visibility_v2_mpiinf3dhp.pth")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_err = evaluate(model, val_loader, device, criterion, args.visibility_loss_weight)
        if val_err < best_val:
            best_val = val_err
