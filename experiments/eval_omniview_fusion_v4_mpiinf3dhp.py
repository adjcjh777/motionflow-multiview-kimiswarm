"""Evaluation script for OmniMultiViewFusionV4 on MPI-INF-3DHP S2/Seq1.
...
(k=2..V). Results are written to JSON and CSV in the same format as the v2/v3
eval scripts.

Usage
-----
    # Full evaluation (requires trained checkpoint + dataset)
    python experiments/eval_omniview_fusion_v4_mpiinf3dhp.py \\
        --checkpoint outputs/omniview_fusion_v4_mpiinf3dhp.pth \\
        --dataset data/webbridge/mpi_inf_3dhp/s_02_seq_01_v14_multiview_m.npz \\
        --run_robustness --run_variable_views

    # Quick CPU/GPU smoke test (synthetic data, fresh model)
    python experiments/eval_omniview_fusion_v4_mpiinf3dhp.py --smoke
"""
...
def build_model(args: argparse.Namespace, n_views: int, j: int) -> OmniMultiViewFusionV4:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate OmniMultiViewFusionV4 on MPI-INF-3DHP S2/Seq1",
    )
    # Inputs
    parser.add_argument("--checkpoint", type=str, default=None, help="Path to trained OmniMultiViewFusionV4 checkpoint")
    ...
    # Outputs
    parser.add_argument("--out_json", type=str, default="outputs/eval_omniview_fusion_v4_mpiinf3dhp.json", help="JSON output path")
    parser.add_argument("--out_csv", type=str, default="outputs/eval_omniview_fusion_v4_mpiinf3dhp.csv", help="CSV output path")
