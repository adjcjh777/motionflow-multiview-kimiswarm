...
def main():
    ...
    parser.add_argument("--cam_aug_schedule", type=str, default="flat", choices=["flat", "extrinsic_curriculum", "intrinsics_curriculum", "extended_curriculum", "extended_intrinsics_curriculum"], help="Camera augmentation schedule")
    ...
    parser.add_argument("--cam_aug_warmup_epochs", type=int, default=0, help="Number of initial epochs with zero camera perturbation for extended_* schedules")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        elif args.cam_aug_schedule in ("extended_curriculum", "extended_intrinsics_curriculum"):
            schedule = schedule_from_args(epoch, args)
            schedule_rot = schedule["rot_std"]
            schedule_trans = schedule["trans_std"]
            schedule_focal = schedule["focal_std"]
            schedule_pp = schedule["pp_std"]
        else:
            schedule_rot = args.cam_aug_rot
            schedule_trans = args.cam_aug_trans
            schedule_focal = args.cam_aug_focal
