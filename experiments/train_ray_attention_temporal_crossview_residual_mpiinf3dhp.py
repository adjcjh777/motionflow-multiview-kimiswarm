"""Train RayAttentionFusionModelTemporalCrossviewResidual on MPI-INF-3DHP clips.
...
that swaps the model for ``RayAttentionFusionModelTemporalCrossviewResidual``, which
replaces the temporal transformer with a joint time + cross-view transformer and adds a
residual refinement head on top of the raw DLT triangulated output.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train temporal + cross-view ray-attention fusion with residual refinement on MPI-INF-3DHP")
