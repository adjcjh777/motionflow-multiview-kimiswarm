"""Train RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointRefined on MPI-INF-3DHP.

Two-stage principal-point correction: coarse correction from raw observations, then
a residual correction from pooled spatio-temporal features before final triangulation.
"""
...
from motionflow_mv.fusion.ray_attention_temporal_crossview_residual_principal_point_refined_model import (
    RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointRefined,
)
...
def main():
    parser = argparse.ArgumentParser(description="Train cross-view + refined PP correction on MPI-INF-3DHP")
    ...
    parser.add_argument("--refined_principal_point_hidden", type=int, default=64)
    parser.add_argument("--refined_principal_point_max_offset", type=float, default=10.0)
    ...
    parser.add_argument("--refined_pp_loss_weight", type=float, default=0.0)
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_crossview_residual_principal_point_refined_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, n_st_layers={args.n_st_layers}, "
          f"residual_hidden={args.residual_hidden}, pp_hidden={args.principal_point_hidden}, "
          f"refined_pp_hidden={args.refined_principal_point_hidden}")

    model = RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointRefined(
        ...
        refined_principal_point_hidden=args.refined_principal_point_hidden,
        refined_principal_point_max_offset=args.refined_principal_point_max_offset,
        focal_max_scale=args.focal_max_scale,
        return_pp_delta=args.pp_loss_weight > 0.0 or args.refined_pp_loss_weight > 0.0 or args.focal_max_scale > 0.0,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, K, R, t in train_loader:
            ...
            # Coarse + refined PP supervision.
            B, T = yb.shape[:2]
            if args.pp_loss_weight > 0.0 or args.refined_pp_loss_weight > 0.0:
                ...
                if args.refined_pp_loss_weight > 0.0:
                    pred_pp_delta_refined = outputs[3]
                    loss = loss + args.refined_pp_loss_weight * criterion(pred_pp_delta_refined, -true_pp_delta)
                if args.focal_max_scale > 0.0:
                    pred_focal_scale = outputs[4] if args.refined_pp_loss_weight > 0.0 else outputs[3]
                    ...
                    focal_loss_weight = args.focal_loss_weight if args.focal_loss_weight is not None else max(args.pp_loss_weight, args.refined_pp_loss_weight)
