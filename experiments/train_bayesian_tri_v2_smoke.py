"""Smoke training for Bayesian triangulation v2 (batched lstsq DLT).

Runs a minimal training loop for ``RayAttentionFusionModelBayesianTriV2`` on a
tiny synthetic multi-view dataset.  The goal is to verify that the model can be
instantiated, its batched DLT head can be differentiated, and a few epochs of
training complete without errors on CPU (or CUDA if available).

Usage
-----
    python experiments/train_bayesian_tri_v2_smoke.py
    python experiments/train_bayesian_tri_v2_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/bayesian_tri_v2_smoke.pth``
"""
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for Bayesian triangulation v2 (batched lstsq DLT)")
    ...
    parser.add_argument("--output", type=str, default="outputs/bayesian_tri_v2_smoke.pth")
