"""Train RayAttentionFusionModelTemporalUncertaintyV2 on MPI-INF-3DHP clips.
...
-----
    conda run -n mf python experiments/train_uncertainty_v2_mpiinf3dhp.py \
        ...
        --clip_len 13 --epochs 30 --uncertainty_weight 0.1

This is a thin wrapper around ``train_ray_attention_temporal_residual_mpiinf3dhp.py``
that swaps the model for ``RayAttentionFusionModelTemporalUncertaintyV2``.  The model
predicts per-view log-variance uncertainties and uses them as weights for DLT
triangulation, then adds the residual refinement head from the temporal residual
model.  The reprojection NLL auxiliary loss is added to the MSE loss.
"""
...
from motionflow_mv.fusion.ray_attention_temporal_uncertainty_v2_model import (
    RayAttentionFusionModelTemporalUncertaintyV2,
)
...
def main():
    parser = argparse.ArgumentParser(description="Train temporal ray-attention fusion with uncertainty-weighted triangulation on MPI-INF-3DHP")
    ...
    parser.add_argument("--uncertainty_weight", type=float, default=0.1, help="Weight for reprojection NLL auxiliary loss")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_uncertainty_v2_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, residual_hidden={args.residual_hidden}, uncertainty_weight={args.uncertainty_weight}")
