"""Train RayAttentionFusionModelTemporalResidual on MPI-INF-3DHP clips.
...
This script is a thin wrapper around ``train_ray_attention_temporal_mpiinf3dhp.py``
that swaps the model for ``RayAttentionFusionModelTemporalResidual``, which adds
a residual refinement head on top of the raw DLT triangulated output.
"""
...
from motionflow_mv.fusion.ray_attention_temporal_residual_v3_model import RayAttentionFusionModelTemporalResidualUncertainty
...
def main():
    parser = argparse.ArgumentParser(description="Train temporal ray-attention fusion with residual refinement on MPI-INF-3DHP")
    ...
    model = RayAttentionFusionModelTemporalResidualUncertainty(
