"""Train RayAttentionFusionModelTemporalResidualCamPEGraph on MPI-INF-3DHP.

Uses an empty skeleton (only cross-view + self-loop edges) because
MPI-INF-3DHP has 28 joints and no official skeleton parent list is shipped.
"""
...
def main():
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_residual_campe_graph_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, residual_hidden={args.residual_hidden}, graph_layers={args.graph_layers}")
    ...
    model = RayAttentionFusionModelTemporalResidualCamPEGraph(
        ...
        residual_hidden=args.residual_hidden, graph_layers=args.graph_layers,
        parents=[], symmetry_pairs=[],  # cross-view only for MPI-INF-3DHP
    ).to(device)
