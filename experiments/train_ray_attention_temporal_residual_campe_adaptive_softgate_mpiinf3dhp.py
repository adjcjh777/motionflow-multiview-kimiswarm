"""Train RayAttentionFusionModelTemporalResidualCamPEAdaptiveSoftGate on MPI-INF-3DHP."""
...
from motionflow_mv.fusion.ray_attention_temporal_residual_campe_adaptive_softgate_model import RayAttentionFusionModelTemporalResidualCamPEAdaptiveSoftGate
...
def main():
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_residual_campe_adaptive_softgate_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, residual_hidden={args.residual_hidden}, target_k={args.target_k}, lambda_gate={args.lambda_gate}")

    model = RayAttentionFusionModelTemporalResidualCamPEAdaptiveSoftGate(
        j=j, d=args.d, n_views=n_views, n_temporal_layers=args.n_temporal_layers,
        residual_hidden=args.residual_hidden, target_k=min(args.target_k, n_views),
        min_views=args.min_views, lambda_gate=args.lambda_gate,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, K, R, t in train_loader:
            ...
            xb = augment_clip(xb)
            optimizer.zero_grad()
            pred, _, _, reg = model(xb, K=K, R=R, t=t)
            loss = criterion(pred, yb)
            if args.lambda_gate > 0.0:
                loss = loss + args.lambda_gate * reg
