"""Train RayAttentionFusionModelTemporalResidualCamPEAdaptive on MPI-INF-3DHP."""
...
from motionflow_mv.fusion.ray_attention_temporal_residual_campe_adaptive_model import RayAttentionFusionModelTemporalResidualCamPEAdaptive
...
def main():
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_residual_campe_adaptive_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, residual_hidden={args.residual_hidden}, k={args.k}")

    model = RayAttentionFusionModelTemporalResidualCamPEAdaptive(
        j=j, d=args.d, n_views=n_views, n_temporal_layers=args.n_temporal_layers,
        residual_hidden=args.residual_hidden, k=min(args.k, n_views),
    ).to(device)
