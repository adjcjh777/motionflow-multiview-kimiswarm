"""Comprehensive evaluation of a trained RayAttentionFusionModelTemporalResidual.

Reports MPJPE, PA-MPJPE, PCK and per-joint/per-view breakdowns on any
WebBridge canonical .npz sequence.  The script mirrors the training dataset
logic so metrics are directly comparable to the training val loop.
...
-------
    conda run -n mf python experiments/eval_ray_attention_temporal_residual_v1.py \
        ...
        --clip_len 13 --batch_size 8 --out outputs/eval_residual_v2.json
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate temporal ray-attention residual model on 3D pose benchmarks"
    )
    ...
    parser.add_argument("--out", type=str, default=None, help="Optional JSON path for metric report")
