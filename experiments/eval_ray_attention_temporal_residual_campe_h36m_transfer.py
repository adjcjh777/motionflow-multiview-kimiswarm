"""Evaluate MPI-trained CamPE model on H36M (cross-dataset / variable views)."""
...
class TemporalClipDataset(torch.utils.data.Dataset):
    def __init__(self, npz_path: str, clip_len: int, stride: int = 1):
        ...
        self.joints_3d = torch.from_numpy(data["joints_3d"]).float() / 1000.0
        self.K = torch.from_numpy(data["camera_K"]).float()
        self.R = torch.from_numpy(data["camera_R"]).float()
        self.t = torch.from_numpy(data["camera_t"]).float() / 1000.0
...
def main():
    ...
    missing, unexpected = model.load_state_dict(torch.load(args.checkpoint, map_location="cpu", weights_only=True), strict=False)
    if missing:
    ...
    print(f"CamPE cross-dataset MPJPE: {mpjpe:.4f}m ({mpjpe*1000:.2f}mm)")
