"""Train a LARGER RayAttentionFusionModelTemporal (d=128, n_temporal_layers=4) on MPI-INF-3DHP.
...
This script is a thin wrapper around the baseline temporal trainer that bumps the
model width to 128 and the temporal transformer depth to 4.  Everything else is
kept identical so that the comparison isolates capacity vs. the baseline (d=64,
n_temporal_layers=2).
"""
...
from motionflow_mv.fusion import ray_attention_model as _ray_attention_model


# The wider/deeper temporal model (d=128, n_temporal_layers=4) triggers a CUDA
# illegal-memory-access inside `torch.linalg.lstsq` for some batches.  Replace
# the solver with a regularised normal-equation solve, which is stable on CUDA
# and keeps gradients on the GPU.
_original_triangulate_joint = _ray_attention_model._triangulate_joint


def _regularised_triangulate_joint(points_2d, weights, proj_matrices):
    """DLT triangulation using regularised normal equations."""
    ...
    for v in range(V):
        ...
        x = points_2d[:, v, 0]
        y = points_2d[:, v, 1]
        A.append(x[:, None, None] * P[:, 2:3, :] - P[:, 0:1, :])
    ...
    sqrt_w = torch.sqrt(w + 1e-6)
    Aw = A3 * sqrt_w
    bw = -a4 * sqrt_w

    AtA = torch.bmm(Aw.transpose(1, 2), Aw)
    Atb = torch.bmm(Aw.transpose(1, 2), bw)
    # Small ridge regularisation keeps the 3x3 system well-conditioned.
    AtA = AtA + 1e-4 * torch.eye(3, device=AtA.device, dtype=AtA.dtype).unsqueeze(0)
    X = torch.linalg.solve(AtA, Atb)
    return X.squeeze(-1)


# Monkey-patch the shared triangulation routine so the larger temporal model can
# avoid the unstable CUDA lstsq path.
_ray_attention_model._triangulate_joint = _regularised_triangulate_joint
...
def main():
    parser = argparse.ArgumentParser(description="Train larger temporal ray-attention fusion on MPI-INF-3DHP")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_d128_mpiinf3dhp.pth")
    ...
    print(f"n_views={n_views}, j={j}, clip_len={args.clip_len}, d={args.d}, n_temporal_layers={args.n_temporal_layers}")
