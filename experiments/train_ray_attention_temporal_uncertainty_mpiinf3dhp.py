"""Train the uncertainty-weighted temporal ray-attention model on MPI-INF-3DHP.

Usage
-----
    conda run -n mf python experiments/train_ray_attention_temporal_uncertainty_mpiinf3dhp.py \
        --train data/webbridge/mpi_inf_3dhp/s_01_seq_01_v14_multiview_m.npz \
                data/webbridge/mpi_inf_3dhp/s_01_seq_02_v14_multiview_m.npz \
        --val data/webbridge/mpi_inf_3dhp/s_02_seq_01_v14_multiview_m.npz \
        --clip_len 13 --epochs 2 --batch_size 8

This script mirrors ``train_ray_attention_temporal_mpiinf3dhp.py`` but uses
``RayAttentionFusionModelTemporalUncertainty``, which predicts per-view/joint
Gaussian uncertainty (log-variance) and feeds it into the weighted DLT head.
"""
...
from motionflow_mv.fusion.ray_attention_temporal_uncertainty_model import RayAttentionFusionModelTemporalUncertainty
...
class RandomClipDataset(torch.utils.data.Dataset):
    """Sample random clips from a sequence."""
...
def main():
    parser = argparse.ArgumentParser(description="Train uncertainty-weighted temporal ray-attention fusion on MPI-INF-3DHP")
    ...
    parser.add_argument("--val_stride", type=int, default=10, help="Validation clip stride for faster eval")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_uncertainty_mpiinf3dhp.pth")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, K, R, t in train_loader:
            ...
            train_nll += nll_loss.item() * xb.size(0)
        ...
        if val_err < best_val:
            ...
            print(f"Epoch {epoch}: loss={train_loss:.6f} (mse={train_mse:.6f}, nll={train_nll:.6f}), "
