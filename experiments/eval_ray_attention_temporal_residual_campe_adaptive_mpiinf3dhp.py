"""Evaluate RayAttentionFusionModelTemporalResidualCamPEAdaptive on MPI-INF-3DHP."""
...
def main():
    ...
    model = RayAttentionFusionModelTemporalResidualCamPEAdaptive(
        j=j, d=args.d, n_views=n_views, n_temporal_layers=args.n_temporal_layers, k=args.k
    ).to(device)
    model.load_state_dict(torch.load(args.checkpoint, map_location="cpu", weights_only=True))

    mpjpe = evaluate(model, loader, device)
    print(f"CamPE+Adaptive MPJPE: {mpjpe:.4f}m ({mpjpe*1000:.2f}mm)")
