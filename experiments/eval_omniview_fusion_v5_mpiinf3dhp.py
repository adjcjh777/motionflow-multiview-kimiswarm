"""Evaluation script for OmniMultiViewFusionV5 on MPI-INF-3DHP S2/Seq1.

Evaluates clean MPJPE / PA-MPJPE, per-joint error, visibility accuracy,
an optional calibration-robustness matrix (rotation, translation, focal,
principal-point perturbations), and an optional variable-view MPJPE@k curve
(k=2..V). Results are written to JSON and CSV in the same format as the v4
eval script.

Usage
-----
    # Full evaluation (requires trained checkpoint + dataset)
    python experiments/eval_omniview_fusion_v5_mpiinf3dhp.py \
        --checkpoint outputs/omniview_fusion_v5_mpiinf3dhp.pth \
        --dataset data/webbridge/mpi_inf_3dhp/s_02_seq_01_v14_multiview_m.npz \
        --run_robustness --run_variable_views \
        --use_camera_view_embedding --use_set_view_aggregator

    # Quick CPU/GPU smoke test (synthetic data, fresh model)
    python experiments/eval_omniview_fusion_v5_mpiinf3dhp.py --smoke \
        --use_camera_view_embedding --use_set_view_aggregator
"""
...
# VariableViewInferenceWrapper is retained only as a fallback helper; the v5
# model is preferred with an explicit ``view_mask`` argument.
from motionflow_mv.fusion.variable_view_inference import (  # noqa: F401
...
# ---------------------------------------------------------------------------
# Synthetic smoke dataset (mirrors the trainer for consistency)
# ---------------------------------------------------------------------------
...
def evaluate_clean(
    ...
    with torch.no_grad():
        for xb, yb, K, R, t in loader:
            ...
            # Visibility accuracy: predicted visible vs. input confidence > 0.
    ...
    # Aggregate visibility accuracy across all clips.
    if vis_preds:
...
def robustness_conditions(smoke: bool = False) -> Dict[str, Dict[str, float]]:
    ...
    if smoke:
        # Reduced matrix for a fast smoke run.
        conditions = {
    ...
    for k in range(min_views, max_views + 1):
        ...
        for subset in subsets:
            ...
            for start, end, x_clip in all_clips:
                ...
                # Build an explicit binary view mask for this subset.  The v5
                # model consumes ``view_mask`` directly in its forward pass so
                # that masked-out views are ignored in confidences, weights,
                # and triangulation.
...
def _convert_for_json(obj: Any) -> Any:
    """Recursively convert numpy arrays/tensors to JSON-serialisable types."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate OmniMultiViewFusionV5 on MPI-INF-3DHP S2/Seq1",
    )
    # Inputs
    ...
    # v5 toggles
    parser.add_argument("--use_camera_view_embedding", action="store_true", help="Use camera-conditioned view embedding instead of learned positional embedding")
    ...
    parser.add_argument("--camera_view_embedding_hidden", type=int, default=32, help="Hidden dimension of the camera-conditioned view embedding MLP")
    ...
    # Outputs
    parser.add_argument("--out_json", type=str, default="outputs/eval_omniview_fusion_v5_mpiinf3dhp.json", help="JSON output path")
    parser.add_argument("--out_csv", type=str, default="outputs/eval_omniview_fusion_v5_mpiinf3dhp.csv", help="CSV output path")
...
def main():
    ...
    # Keep per-joint arrays in the JSON report for downstream analysis.
    ...
    if args.run_variable_views:
        if args.smoke or data_npz is not None:
            if args.smoke:
                # Re-use synthetic arrays for the variable-view test.
