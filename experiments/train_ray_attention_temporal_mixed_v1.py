"""Mixed-dataset training for RayAttentionFusionModelTemporal.

Trains a single shared temporal model on MPI-INF-3DHP + Human3.6M clips.
Human3.6M clips are padded to the MPI-INF-3DHP view/joint grid and routed through
a dataset-specific output head (see ``ray_attention_temporal_model_mixed_v1.py``).
...
# Shared canonical dimensions inferred from MPI-INF-3DHP (the larger dataset).
MAX_VIEWS = 14
MAX_JOINTS = 28
...
class MixedTemporalDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.dataset_id = self.DATASET_IDS[dataset_name]
    ...
    def __getitem__(self, idx):
        ...
        x = torch.cat(
            [self.points_2d[start:end], self.confidences[start:end].unsqueeze(-1)], dim=-1
        )  # (T, V, J, 3)
...
def main():
    parser = argparse.ArgumentParser(description="Train mixed-dataset temporal ray-attention fusion")
    parser.add_argument("--mpi_train", type=str, nargs="+", required=True, help="MPI-INF-3DHP train .npz files")
    parser.add_argument("--h36m_train", type=str, nargs="+", required=True, help="Human3.6M train .npz files")
    ...
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_mixed_v1.pth")
    ...
    for p in args.mpi_train:
        train_datasets.append(
            MixedTemporalDataset(p, "mpi", args.clip_len, n_samples=args.train_samples)
        )
    for p in args.h36m_train:
        train_datasets.append(
            MixedTemporalDataset(p, "h36m", args.clip_len, n_samples=args.train_samples)
        )
    train_dataset = torch.utils.data.ConcatDataset(train_datasets)
    val_dataset = MixedTemporalDataset(args.val, "mpi", args.clip_len, n_samples=None, stride=1)
    ...
    model = RayAttentionFusionModelTemporalMixed(
        d=args.d, n_temporal_layers=args.n_temporal_layers
    ).to(device)
