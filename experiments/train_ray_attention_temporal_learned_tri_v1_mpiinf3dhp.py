"""Train RayAttentionFusionModelTemporalResidualLearnedTri on MPI-INF-3DHP.

This script is a thin wrapper around ``train_ray_attention_temporal_residual_mpiinf3dhp.py``
that swaps the model for ``RayAttentionFusionModelTemporalResidualLearnedTri``,
which replaces the weighted DLT triangulation step with a differentiable
Gauss-Newton optimizer using the network-predicted per-view weights.
...
def main():
    parser = argparse.ArgumentParser(
        description="Train temporal ray-attention fusion with learnable Gauss-Newton triangulation on MPI-INF-3DHP"
    )
    ...
    parser.add_argument("--checkpoint", type=str, default=None, help="Optional checkpoint to load (residual DLT model)")
    parser.add_argument("--output", type=str, default="outputs/ray_attention_temporal_learned_tri_v1.pth")
    ...
    print(
        ...
        f"gn_iters={args.gn_iters}, gn_damping={args.gn_damping}"
    )
