"""Smoke training for the kinematic-chain-constraints prototype.

Trains ``RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointKinematicChain``
on a tiny synthetic multi-view dataset.  In addition to the standard MPJPE pose loss, a
kinematic-chain regularisation loss (bone-length consistency + symmetry plane) is added
to enforce anatomical constraints.

Usage
-----
    python experiments/prototypes/iter17_kinematic-chain-constraints_smoke.py
    python experiments/prototypes/iter17_kinematic-chain-constraints_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/iter17_kinematic_chain_constraints_smoke.pth``
"""
...
from motionflow_mv.fusion.ray_attention_temporal_crossview_residual_principal_point_kinematic_chain_model import (  # noqa: E501
...
from motionflow_mv.losses.kinematic_chain import kinematic_chain_loss
...
def evaluate(model, loader, device, criterion, bone_weight: float = 0.1):
    model.eval()
    total_loss = 0.0
    total_err = 0.0
    count = 0
    with torch.no_grad():
        for xb, yb, K, R, t in loader:
            ...
            loss = loss + bone_weight * kinematic_chain_loss(
                pred, yb, H36M_17_PARENTS, H36M_17_SYMMETRY_PAIRS
            )
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for kinematic-chain-constraints prototype")
    ...
    parser.add_argument("--kc_hidden_dim", type=int, default=32, help="Kinematic-chain graph hidden size")
    parser.add_argument("--kc_num_layers", type=int, default=2, help="Kinematic-chain graph layers")
    parser.add_argument("--bone_weight", type=float, default=0.1, help="Weight for kinematic-chain loss")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use (cpu or cuda)")
    parser.add_argument("--output", type=str, default="outputs/iter17_kinematic_chain_constraints_smoke.pth")
    ...
    model = RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointKinematicChain(
        ...
        kc_hidden_dim=args.kc_hidden_dim,
        kc_num_layers=args.kc_num_layers,
    ).to(device)
    ...
    print(
        ...
        f"kc_hidden_dim={args.kc_hidden_dim}, kc_num_layers={args.kc_num_layers}, "
        f"params={n_params}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, Kb, Rb, tb in train_loader:
            ...
            loss = criterion(pred, yb)
            loss = loss + args.bone_weight * kinematic_chain_loss(
                pred, yb, H36M_17_PARENTS, H36M_17_SYMMETRY_PAIRS
            )
            loss.backward()
            optimizer.step()
        ...
        val_loss, val_err = evaluate(model, val_loader, device, criterion, args.bone_weight)
        if val_err < best_val:
            best_val = val_err
