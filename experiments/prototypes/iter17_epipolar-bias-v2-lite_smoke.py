"""Smoke training for epipolar-bias-v2-lite (iter-17).

Trains ``RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointEpipolarBiasV2Lite``
on a tiny synthetic multi-view dataset.  The model injects epipolar-geometry bias
only into the final spatio-temporal transformer layer, making it a lighter sibling
of the full epipolar-bias-v2 model.

Usage
-----
    python experiments/prototypes/iter17_epipolar-bias-v2-lite_smoke.py
    python experiments/prototypes/iter17_epipolar-bias-v2-lite_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/iter17_epipolar_bias_v2_lite_smoke.pth``
"""
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for epipolar-bias-v2-lite (iter-17)")
    ...
    parser.add_argument("--output", type=str, default="outputs/iter17_epipolar_bias_v2_lite_smoke.pth")
    ...
    print(
        ...
        f"principal_point_hidden={args.principal_point_hidden}, params={n_params}"
    )
