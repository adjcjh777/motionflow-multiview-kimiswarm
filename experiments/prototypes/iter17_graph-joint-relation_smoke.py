"""Smoke training for Graph Joint Relation (iter-17).

Trains ``RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointGraph``
on a tiny synthetic multi-view dataset.  This is a CPU-only sanity check for the
graph-joint-relation idea: the dense joint-level self-attention in the base
ray-attention model is replaced by skeleton-graph message passing over a
(view, joint) graph.

Usage
-----
    python experiments/prototypes/iter17_graph-joint-relation_smoke.py
    python experiments/prototypes/iter17_graph-joint-relation_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/iter17_graph_joint_relation_smoke.pth``
"""
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for Graph Joint Relation (iter-17)")
    ...
    parser.add_argument("--graph_num_layers", type=int, default=1, help="Number of graph-joint-relation layers")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use (cpu or cuda)")
    parser.add_argument("--output", type=str, default="outputs/iter17_graph_joint_relation_smoke.pth")
    ...
    print(
        ...
        f"n_st_layers={args.n_st_layers}, graph_num_layers={args.graph_num_layers}, "
        f"params={n_params}"
    )
