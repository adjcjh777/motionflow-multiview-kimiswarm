"""Smoke training for attention-entropy regularization on MPI-INF-3DHP.

Trains ``RayAttentionFusionModelHierarchicalAttentionEntropyReg`` on a tiny
synthetic multi-view sequence.  The smoke is CPU-only, uses small dimensions
(``d=32``, ``batch_size=2``, ``clip_len=9``), and runs for at most three epochs.

Usage
-----
    python experiments/prototypes/iter17_attention-entropy-regularization_smoke.py
    python experiments/prototypes/iter17_attention-entropy-regularization_smoke.py --epochs 3

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/iter17_attention_entropy_reg_smoke.pth``
"""
...
def evaluate(model, loader, device, criterion):
    ...
    with torch.no_grad():
        for xb, yb, K, R, t in loader:
            ...
            pred, weights, pp_delta, entropy_loss = model(xb, K=K, R=R, t=t)
            loss = criterion(pred, yb) + entropy_loss
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for attention-entropy regularization")
    ...
    parser.add_argument("--attention_entropy_weight", type=float, default=0.01, help="Entropy regularization weight")
    parser.add_argument("--entropy_temperature", type=float, default=1.0, help="Softmax temperature for entropy")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use (cpu or cuda)")
    parser.add_argument("--output", type=str, default="outputs/iter17_attention_entropy_reg_smoke.pth")
    ...
    print(
        ...
        f"attention_entropy_weight={args.attention_entropy_weight}, "
        f"entropy_temperature={args.entropy_temperature}, params={n_params}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, Kb, Rb, tb in train_loader:
            ...
            optimizer.zero_grad()
            pred, weights, pp_delta, entropy_loss = model(xb, K=Kb, R=Rb, t=tb)
            loss = criterion(pred, yb) + entropy_loss
            loss.backward()
            optimizer.step()
