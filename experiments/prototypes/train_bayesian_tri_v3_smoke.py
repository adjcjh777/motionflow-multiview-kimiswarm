"""Smoke training for Bayesian triangulation v3.

Trains ``RayAttentionFusionModelBayesianTriV3`` on the same tiny synthetic
multi-view dataset used by the v2 smoke script.  In addition to the MSE pose
loss, the script demonstrates how to use the per-joint 3-D precision matrix by
adding a 3-D Gaussian negative-log-likelihood term that encourages the
predicted precision to explain the pose error.

Usage
-----
    python experiments/prototypes/train_bayesian_tri_v3_smoke.py
    python experiments/prototypes/train_bayesian_tri_v3_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/bayesian_tri_v3_smoke.pth``
"""
...
def precision_nll_loss(pred: torch.Tensor, target: torch.Tensor, L3d: torch.Tensor) -> torch.Tensor:
    """3-D Gaussian NLL using the predicted Cholesky precision factor.

    Args:
        pred:   (B, T, J, 3)
        target: (B, T, J, 3)
        L3d:    (B, T, J, 3, 3) lower-triangular Cholesky factor of the
                information (precision) matrix.

    Returns:
        Scalar NLL.
    """
    residual = (pred - target).unsqueeze(-1)  # (B, T, J, 3, 1)
    # Λ = L L^T; Mahalanobis term = r^T Λ r = ||L^T r||^2
    Lt_r = torch.matmul(L3d.transpose(-2, -1), residual)
    mahal = (Lt_r.squeeze(-1) ** 2).sum(dim=-1).mean()
    # log determinant of Λ = 2 * sum(log(diag(L)))
    log_det = 2.0 * torch.log(torch.diagonal(L3d, dim1=-2, dim2=-1).clamp(min=1e-4)).sum(dim=-1).mean()
...
def evaluate(model, loader, device, criterion):
    ...
    with torch.no_grad():
        for xb, yb, K, R, t in loader:
            ...
            pred, _, _, L3d = model(xb, K=K, R=R, t=t)[:4]
            loss = criterion(pred, yb) + precision_nll_loss(pred, yb, L3d)
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for Bayesian triangulation v3")
    ...
    parser.add_argument("--joint_precision_hidden", type=int, default=64, help="Per-joint precision head hidden")
    parser.add_argument("--refinement_hidden", type=int, default=64, help="Refinement head hidden")
    parser.add_argument("--n_refinement_iters", type=int, default=2, help="Number of iterative refinement steps")
    parser.add_argument("--gn_iters", type=int, default=2, help="Gauss-Newton iterations")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use (cpu or cuda)")
    parser.add_argument("--output", type=str, default="outputs/bayesian_tri_v3_smoke.pth")
    ...
    model = RayAttentionFusionModelBayesianTriV3(
        ...
        joint_precision_hidden=args.joint_precision_hidden,
        refinement_hidden=args.refinement_hidden,
        n_refinement_iters=args.n_refinement_iters,
    ...
    print(
        ...
        f"gn_iters={args.gn_iters}, n_refinement_iters={args.n_refinement_iters}, "
        f"params={n_params}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb, Kb, Rb, tb in train_loader:
            ...
            optimizer.zero_grad()
            pred, _, _, L3d = model(xb, K=Kb, R=Rb, t=tb)[:4]
            loss = criterion(pred, yb) + precision_nll_loss(pred, yb, L3d)
            loss.backward()
            optimizer.step()
