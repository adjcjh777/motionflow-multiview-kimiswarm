"""Smoke training for the deeper spatio-temporal attention prototype.

Trains ``DeeperStAttentionPrincipalPointModel`` on the same tiny synthetic
multi-view dataset used by ``train_bayesian_tri_v3_smoke.py``.  The model stacks
interleaved residual T-V-J TransformerEncoderLayer blocks on top of the
principal-point-corrected per-frame encoder.

Usage
-----
    python experiments/prototypes/iter17_deeper-st-attention_smoke.py
    python experiments/prototypes/iter17_deeper-st-attention_smoke.py --epochs 2 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/iter17_deeper_st_attention_smoke.pth``
"""
...
_spec = importlib.util.spec_from_file_location(
    "_deeper_st_attention_model",
    str(Path(__file__).parent / "deeper_st_attention_model" / "deeper_st_attention_model.py"),
)
_deeper_st_attention_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_deeper_st_attention_module)
DeeperStAttentionPrincipalPointModel = _deeper_st_attention_module.DeeperStAttentionPrincipalPointModel
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for deeper ST attention model")
    ...
    parser.add_argument("--n_st_blocks", type=int, default=2, help="Number of interleaved ST attention blocks")
    parser.add_argument("--n_heads", type=int, default=4, help="Attention heads (must divide d)")
    parser.add_argument("--residual_hidden", type=int, default=64, help="Residual MLP hidden size")
    parser.add_argument("--principal_point_hidden", type=int, default=32, help="Principal-point MLP hidden size")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use (cpu or cuda)")
    parser.add_argument("--output", type=str, default="outputs/iter17_deeper_st_attention_smoke.pth")
    ...
    print(
        ...
        f"n_st_blocks={args.n_st_blocks}, n_heads={args.n_heads}, "
        f"params={n_params}"
    )
