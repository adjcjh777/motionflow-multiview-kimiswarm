"""Smoke training for adaptive-scale cross-view spatial pyramid.

Trains ``RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointAdaptiveSpatialPyramid``
on the same tiny synthetic multi-view dataset used by the other prototype smoke
scripts.  The run is CPU-only and uses tiny dimensions so it can finish in a few
seconds while still exercising the adaptive pyramid gate and the full forward
pass.

Usage
-----
    python experiments/prototypes/iter17_adaptive_scale_spatial_pyramid_smoke.py
    python experiments/prototypes/iter17_adaptive_scale_spatial_pyramid_smoke.py --epochs 3 --batch_size 2

Output
------
    - prints per-epoch train/val metrics
    - saves a checkpoint to ``outputs/iter17_adaptive_scale_spatial_pyramid_smoke.pth``
"""
...
from motionflow_mv.fusion.ray_attention_temporal_crossview_residual_principal_point_adaptive_spatial_pyramid_model import (
    RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointAdaptiveSpatialPyramid,
)
...
def main():
    parser = argparse.ArgumentParser(description="Smoke training for adaptive-scale cross-view spatial pyramid")
    ...
    parser.add_argument("--output", type=str, default="outputs/iter17_adaptive_scale_spatial_pyramid_smoke.pth")
    ...
    model = RayAttentionFusionModelTemporalCrossviewResidualPrincipalPointAdaptiveSpatialPyramid(
        j=n_joints,
        d=args.d,
        n_views=4,
        ...
        spatial_pyramid_scales=(1, 2, 4),
        spatial_pyramid_heads=1,
    ).to(device)
