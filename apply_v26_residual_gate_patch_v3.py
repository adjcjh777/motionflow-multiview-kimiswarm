...
def main() -> None:
    # Ensure we are on the correct branch from current HEAD.
    run(["git", "checkout", "-B", "swarm/v26_temporal_geometry_fusion_design"])
...
'''
    ...
    # smoke script
    smoke_script = "scripts/run_v26_temporal_geometry_fusion_smoke_warmstart.sh"
    smoke_content = '''#!/usr/bin/env bash
# v26 temporal geometry fusion smoke with warm-started residual gate.
# The temporal attention residual gate is initialised at 0.0 so the model
# starts from the per-frame v25 path and learns to open the temporal gate.
set -euo pipefail
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \\
    ...
    --v26_temporal_attention_residual_gate_init 0.0 \\
    ...
    --epochs 5 --batch_size 16 --train_samples 500 --val_stride 10 \\
    ...
    --output outputs/omniview_fusion_v26_temporal_geometry_fusion_smoke_warmstart.pth \\
    > outputs/omniview_fusion_v26_temporal_geometry_fusion_smoke_warmstart.log 2>&1
'''
    ...
    run(["git", "commit", "-m", "v26: add warm-startable residual gate to temporal geometry attention\n\n"
