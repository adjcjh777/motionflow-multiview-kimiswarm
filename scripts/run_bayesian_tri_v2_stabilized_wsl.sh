#!/usr/bin/env bash
# Stabilized large-scale MPI-INF-3DHP training for Bayesian triangulation v2.
# Addresses the divergence seen in the first large run by adding cosine LR,
# warmup, gradient clipping, AMP, and a lower base learning rate.
set -euo pipefail
cd "$(dirname "$0")/.."
...
python -u experiments/train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py \
  ...
  --output outputs/bayesian_tri_v2_stabilized_mpiinf3dhp.pth
