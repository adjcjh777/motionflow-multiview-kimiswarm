#!/usr/bin/env bash
# v27 uncertainty-aware depth-proposal smoke test on the local RTX 4090.
# Runs v25 geometry fusion + v27 UDP for a tiny number of epochs/samples so the
# effect of the learned depth distribution can be compared against the matching
# baseline (scripts/run_v25_baseline_smoke_local_4090.sh).
set -euo pipefail

CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
PY=${PYTHON:-python}
OUTPUT=${OUTPUT:-outputs/omniview_fusion_v27_udp_smoke_local_4090.pth}
LOG=${LOG:-outputs/omniview_fusion_v27_udp_smoke_local_4090.log}

$PY -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    --use_mixed_loader \
    ...
    --epochs 3 --batch_size 16 --train_samples 500 --val_stride 10 \
