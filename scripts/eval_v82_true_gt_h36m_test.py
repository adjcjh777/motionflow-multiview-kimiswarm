"""Evaluate a v82 OmniMultiViewFusionV5 checkpoint on true-GT H36M test subjects.

The v82 checkpoint was trained with the WebBridge mixed loader (``n_views=14``).
...
Usage
-----
    python scripts/eval_v82_true_gt_h36m_test.py \
        --checkpoint outputs/ablations/v82_true_gt_v2_medium_a800.pth \
        --config_json outputs/ablations/v82_true_gt_v2_medium_a800.config.json

The script prints per-subject MPJPE / PA-MPJPE and writes the combined result
to the JSON path given by ``--out_json``.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate v82 checkpoint on true-GT H36M test subjects S9/S11",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="outputs/ablations/v82_true_gt_v2_medium_a800.pth",
        help="Path to v82 .pth checkpoint",
    )
    parser.add_argument(
        "--config_json",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=13,
        help="Stride for test clips. Use 1 for dense eval, 13 to match prior v82 test reporting.",
    )
    parser.add_argument(
        "--out_json",
        type=str,
        default="outputs/eval_v82_true_gt_v2_h36m_test.json",
