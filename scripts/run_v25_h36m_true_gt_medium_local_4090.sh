#!/usr/bin/env bash
# Local RTX 4090 medium run for v25 (geometry fusion) on the NON-CIRCULAR
# H36M true-GT standard protocol (S1,5,6,7,8 train -> S9/S11 val; issue #194).
#
# Completion-criterion anchor (CLAUDE.md): on the true-GT protocol, DLT must
# land at 15-30 mm MPJPE (done: S9 29.54 / S11 21.81 mm) AND v25 must land in
# a plausible range (tens of mm, same order, no NaN). This script runs v25
# with its verified flag set (v25 geometry fusion + DLT reweight stack) at a
# scale that fits the local 4090 (8 epochs, 1024 samples/epoch).
set -euo pipefail
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --output outputs/omniview_fusion_v25_h36m_true_gt_medium.pth \
    > outputs/omniview_fusion_v25_h36m_true_gt_medium.log 2>&1
