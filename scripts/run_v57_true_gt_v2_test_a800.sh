#!/usr/bin/env bash
# Evaluate v57 true-GT v2 medium checkpoint on H36M test subjects (S9/S11).
#
# Usage (on A800):
#   bash scripts/run_v57_true_gt_v2_test_a800.sh
#
#   # Specify GPU (project policy: only GPU 6 or 7)
#   CUDA_VISIBLE_DEVICES=7 bash scripts/run_v57_true_gt_v2_test_a800.sh
...
"${PYTHON}" -u scripts/eval_v57_true_gt_h36m_test.py \
    ...
    --out_json outputs/eval_v57_true_gt_v2_h36m_test.json \
    --batch_size 8 \
    --val_stride 13 \
    > outputs/eval_v57_true_gt_v2_h36m_test.log 2>&1
