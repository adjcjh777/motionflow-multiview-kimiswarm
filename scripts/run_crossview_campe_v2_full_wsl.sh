#!/usr/bin/env bash
# Train cross-view CamPE v2 full model on MPI-INF-3DHP (WSL + RTX 4090).
set -euo pipefail
cd "$(dirname "$0")/.."
...
python -u experiments/train_ray_attention_temporal_crossview_residual_mpiinf3dhp.py \
  ...
  --epochs 10 --train_samples 1000 --batch_size 8 \
  --output outputs/ray_attention_temporal_crossview_residual_campe_v2_full.pth \
  "$@"
