#!/usr/bin/env bash
# Variable-view inference with DLT fallback for v85 random-view-dropout A800
# checkpoint on the true-GT v2 validation set.
set -euo pipefail

export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-6}

cd /mnt/nvme0n1p1/zhangzy/motionflow-multiview-kimiswarm-iter20

CKPT="outputs/ablations/v85_random_view_dropout_medium_a800.pth"
CONFIG="outputs/ablations/v85_random_view_dropout_medium_a800.config.json"
MANIFEST="tmp/h36m_true_gt_v2_val_manifest.txt"
OUT_PREFIX="outputs/variable_view_fix/variable_view_v85_random_view_dropout_true_gt_v2_medium_a800_dlt_fallback"

if [[ ! -f "$CKPT" ]]; then
    echo "ERROR: Checkpoint not found: $CKPT" >&2
    exit 1
fi
if [[ ! -f "$CONFIG" ]]; then
    echo "ERROR: Config not found: $CONFIG" >&2
    exit 1
fi
if [[ ! -f "$MANIFEST" ]]; then
    echo "ERROR: Manifest not found: $MANIFEST" >&2
    exit 1
fi

mkdir -p "outputs/variable_view_fix"

PYTHONUNBUFFERED=1 /mnt/nvme0n1p1/zhangzy/motionflow-multiview-kimiswarm/.venv/bin/python experiments/eval_variable_views.py \
    --model_class omniview_v5 \
    --checkpoint "$CKPT" \
    --config "$CONFIG" \
    --dataset_manifest "$MANIFEST" \
    --clip_len 13 \
    --min_views 2 \
    --max_views 4 \
    --num_subsets_per_k 50 \
    --seed 42 \
    --var_view_dlt_fallback \
    --output_csv "${OUT_PREFIX}.csv" \
    --output_json "${OUT_PREFIX}.json" \
    > "${OUT_PREFIX}.log" 2>&1

echo "v85 true-GT v2 A800 variable-view DLT-fallback eval complete. Results: ${OUT_PREFIX}.{csv,json}"
