#!/usr/bin/env bash
# v26 + UDP local 4090 run: temporal geometry fusion with uncertainty-aware depth proposals.
# Mirrors run_v26_temporal_geometry_fusion_a800_smoke_fast.sh but enables UDP and runs
# a meaningful number of epochs for comparison against the v26 baseline.
set -euo pipefail

CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
PYTHON=${PYTHON:-python}
OUTPUT=${OUTPUT:-outputs/omniview_fusion_v26_udp_local_4090.pth}
LOG=${LOG:-outputs/omniview_fusion_v26_udp_local_4090.log}
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --epochs 20 --batch_size 16 --train_samples 4000 --val_stride 10 \
