#!/usr/bin/env bash
# Full cross-view temporal residual + principal-point correction model on WSL + RTX 4090.
set -euo pipefail
cd "$(dirname "$0")/.."
...
python -u experiments/train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py \
  ...
  --output outputs/ray_attention_temporal_crossview_residual_principal_point_full.pth \
  "$@"
