#!/usr/bin/env bash
# Launch v30 hardened hierarchical + physical loss run on A800-D.
set -euo pipefail
...
launch_if_free() {
    ...
    tmux new-session -d -s "$session" -n v30 "cd $REPO && CUDA_VISIBLE_DEVICES=$gpu python3 -u $BASE $COMMON_FLAGS $extra > outputs/omniview_fusion_${session}.log 2>&1"
    echo "Launched $session on GPU $gpu."
}

# v30a: hardened hierarchical encoder + physical loss, no TTE.
launch_if_free v30a_hierarchical_physical_a800 0 "--use_hierarchical_multiview_v30 --v30_n_part_layers 2 --v30_stochastic_depth_prob 0.1 --use_physical_space_temporal_loss_v29 --v29_floor_loss_weight 0.01 --v29_bone_temporal_weight 0.01 --v29_com_jitter_weight 0.001 --output outputs/omniview_fusion_v30a_hierarchical_physical_a800.pth"
