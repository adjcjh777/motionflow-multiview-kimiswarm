#!/usr/bin/env bash
# Local RTX 4090 smoke test for v80 learned view-reliability before
# triangulation (VRBT) using only non-circular labels (MPI-INF-3DHP + Shelf/Campus).
# H36M is excluded because its 3D labels are circular (DLT triangulation of 2D).
#
# v50/v51 auxiliary losses and v57 DC-PSC are disabled so v80 can be isolated.
#
...
python -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --v80_vrbt_use_geometry_bias \
    --v80_vrbt_use_feature_bias \
    --v80_vrbt_identity_init \
    ...
    --output outputs/omniview_fusion_v80_mpi_only_noncircular_smoke.pth \
    > outputs/omniview_fusion_v80_mpi_only_noncircular_smoke.log 2>&1
