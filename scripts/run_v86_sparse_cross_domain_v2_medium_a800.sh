#!/usr/bin/env bash
# A800-local launch of v86 sparse-view / cross-domain medium run on the
# corrected H36M true-GT v2 protocol.  This script is meant to run *on* the
# A800 host, not via SSH from WSL.
#
# Usage (on A800):
#   CUDA_VISIBLE_DEVICES=6 bash scripts/run_v86_sparse_cross_domain_v2_medium_a800.sh
...
# Default to GPU 6 if not set.
GPU_ID="${CUDA_VISIBLE_DEVICES:-6}"

cd "${REPO}"
mkdir -p outputs/ablations

CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON}" -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    --use_mixed_loader \
