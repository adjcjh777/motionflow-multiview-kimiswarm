#!/usr/bin/env bash
# Local RTX 4090 MPI-only non-circular smoke for v25 geometry fusion.
# Uses only MPI-INF-3DHP (true 3D GT, 14 views) to avoid mixed-view padding
# issues with Shelf/Campus in compute_epipolar_distance.
set -euo pipefail

PYTHON=${PYTHON:-python}
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    --use_mixed_loader \
    --mixed_manifest configs/splits/mpi_only_noncircular_smoke.yaml \
    ...
    --epochs 3 --batch_size 2 --train_samples 128 --val_stride 10 \
    ...
    --output outputs/omniview_fusion_v25_mpi_only_noncircular_smoke.pth \
    > outputs/omniview_fusion_v25_mpi_only_noncircular_smoke.log 2>&1
