#!/usr/bin/env bash
# Train cross-view adaptive view selection small model on MPI-INF-3DHP (WSL + RTX 4090).
set -euo pipefail
cd "$(dirname "$0")/.."
...
python -u experiments/train_ray_attention_temporal_crossview_residual_mpiinf3dhp.py \
  ...
  --epochs 10 --train_samples 500 --batch_size 8 --selector_k 4 \
  --output outputs/ray_attention_temporal_crossview_residual_adaptive_small.pth \
  "$@"
