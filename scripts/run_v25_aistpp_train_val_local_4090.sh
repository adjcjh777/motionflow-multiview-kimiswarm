#!/usr/bin/env bash
# Local RTX 4090 v25 (geometry fusion) training on AIST++ full train/val split.
# Uses configs/splits/aistpp_train_val.yaml (9 views, 17 joints).
# Medium run: 8 epochs, 1024 samples/epoch, batch size 16.
set -euo pipefail

PYTHON=${PYTHON:-python}
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    --use_mixed_loader \
    --mixed_manifest configs/splits/aistpp_train_val_mixed.yaml \
    --num_domains 8 \
    ...
    --epochs 8 --batch_size 8 --train_samples 1024 --val_stride 20 \
    ...
    --output outputs/omniview_fusion_v25_aistpp_train_val.pth \
    > outputs/omniview_fusion_v25_aistpp_train_val.log 2>&1
