#!/usr/bin/env bash
# v31 domain-balanced sampling smoke test on local RTX 4090.
# Uses the v5 trainer wrapper that swaps the WebBridge mixed loader for a
# DomainBalancedSampler.  Physical loss is enabled with a warmup.
set -euo pipefail
export PYTHONUNBUFFERED=1
PYTHON=${PYTHON:-python}
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi_v31_domain_balanced.py \
    --use_mixed_loader \
    ...
    --output outputs/omniview_fusion_v31_domain_balanced_smoke_local_4090.pth \
    > outputs/omniview_fusion_v31_domain_balanced_smoke_local_4090.log 2>&1 &
echo $! > outputs/v31_domain_balanced_smoke_local_4090.pid
wait $!
