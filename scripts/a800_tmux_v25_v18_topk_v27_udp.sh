#!/usr/bin/env bash
# A800 tmux launcher for v25 + v18 top-k ST + v27 UDP full-scale run.
set -euo pipefail

SESSION="v25_v18_topk_v27_udp_gpu4"
LOG="outputs/omniview_fusion_v25_v18_topk_v27_udp_a800.log"

tmux has-session -t "$SESSION" 2>/dev/null && { echo "Session $SESSION already exists"; exit 0; }
...
tmux send-keys -t "$SESSION" "CUDA_VISIBLE_DEVICES=4 python -u experiments/train_omniview_fusion_v5_webbridge_multi.py \\
    ...
    --use_multiview_geometry_fusion_v25 --use_uncertainty_depth_proposals_v27 --v27_udp_n_mixtures 1 \\
    --v25_geom_loss_weight 0.1 --v25_use_geometry_attention --v25_use_learned_depth_triangulation --v25_use_geometry_bundle_adjustment \\
    --num_workers 4 --d 128 --residual_hidden 256 --n_st_layers 3 \\
    --graph_num_layers 1 --n_joint_layers 1 --n_heads 4 \\
    --epochs 20 --batch_size 32 --train_samples -1 --val_stride 10 \\
    ...
    --output outputs/omniview_fusion_v25_v18_topk_v27_udp_a800.pth > $LOG 2>&1" C-m

echo "A800 tmux session $SESSION started. Attach with: tmux attach -t $SESSION"
