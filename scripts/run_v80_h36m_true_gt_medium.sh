#!/usr/bin/env bash
# Local RTX 4090 medium run for v80 (view-reliability weighting) on the
# NON-CIRCULAR H36M true-GT standard protocol (S1,5,6,7,8 train -> S9/S11
# val; issue #194).
#
# Scales the verified v80 smoke wiring
# (scripts/run_v80_h36m_true_gt_smoke_local_4090.sh) to a medium training
# run: 8 epochs, 1024 samples/epoch, batch size 16.  Uses the same v80
# architecture flags and mixed-loader split as the smoke/long recipes.
#
# Sanity anchors: DLT baseline on these labels = S9 29.54 / S11 21.81 mm
# (data/h36m_true_gt/dlt_baseline_h36m.json).  Goal: learned model val MPJPE
# in the tens of mm, monotonically falling, no NaN.
#
# Run only when the local GPU is free (project rule: at most ONE training
# task at a time on the RTX 4090).
set -euo pipefail
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    > outputs/omniview_fusion_v80_h36m_true_gt_medium.log 2>&1
