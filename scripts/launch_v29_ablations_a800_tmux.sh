...
launch_if_free() {
    ...
    tmux new-session -d -s "$session" -n v29 "cd $REPO && CUDA_VISIBLE_DEVICES=$gpu $cmd"
    echo "Launched $session on GPU $gpu."
}

# v29a: hierarchical encoder only (no TTE, no physical loss).
launch_if_free v29a_gpu1 1 "python -u $BASE $common_flags --use_hierarchical_multiview_v29 --output outputs/omniview_fusion_v29a_hierarchical_only_a800.pth > outputs/omniview_fusion_v29a_hierarchical_only_a800.log 2>&1"

# v29b: hierarchical encoder + TTE.
launch_if_free v29b_gpu2 2 "python -u $BASE $common_flags --use_hierarchical_multiview_v29 --use_test_time_self_evolution_v29 --output outputs/omniview_fusion_v29b_hierarchical_tte_a800.pth > outputs/omniview_fusion_v29b_hierarchical_tte_a800.log 2>&1"

# v29d: TTE + physical temporal loss, no hierarchical encoder.
launch_if_free v29d_gpu3 3 "python -u $BASE $common_flags --use_test_time_self_evolution_v29 --use_physical_space_temporal_loss_v29 --v29_floor_loss_weight 0.01 --v29_bone_temporal_weight 0.01 --v29_com_jitter_weight 0.001 --output outputs/omniview_fusion_v29d_tte_physical_only_a800.pth > outputs/omniview_fusion_v29d_tte_physical_only_a800.log 2>&1"
