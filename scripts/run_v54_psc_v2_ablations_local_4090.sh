#!/usr/bin/env bash
# Local RTX 4090 ablation matrix for v54 Physical-Space Calibration v2.
#
# Runs the v54 PSC-v2 medium config four times, disabling one component at a
# time to isolate contribution:
#   1. no_floor    (contact/bone/gnn still active)
#   2. no_contact  (floor/bone/gnn still active)
#   3. no_bone     (floor/contact/gnn still active)
#   4. no_gnn      (floor/contact/bone still active; uses per-joint MLP fallback)
#
...
# no_floor
necho "Running v54 ablation: no_floor"
python -u experiments/train_omniview_fusion_v5_webbridge_multi.py "${BASE_FLAGS[@]}" \
    --no_v54_psc2_use_floor \
    --output outputs/omniview_fusion_v54_psc_v2_abl_no_floor_local_4090.pth \
    >> outputs/omniview_fusion_v54_psc_v2_abl_no_floor_local_4090.log 2>&1

# no_contact
echo "Running v54 ablation: no_contact"
python -u experiments/train_omniview_fusion_v5_webbridge_multi.py "${BASE_FLAGS[@]}" \
    --no_v54_psc2_use_contact \
    --output outputs/omniview_fusion_v54_psc_v2_abl_no_contact_local_4090.pth \
    >> outputs/omniview_fusion_v54_psc_v2_abl_no_contact_local_4090.log 2>&1

# no_bone
echo "Running v54 ablation: no_bone"
python -u experiments/train_omniview_fusion_v5_webbridge_multi.py "${BASE_FLAGS[@]}" \
    --no_v54_psc2_use_bone_scale \
    --output outputs/omniview_fusion_v54_psc_v2_abl_no_bone_local_4090.pth \
    >> outputs/omniview_fusion_v54_psc_v2_abl_no_bone_local_4090.log 2>&1

# no_gnn
echo "Running v54 ablation: no_gnn"
python -u experiments/train_omniview_fusion_v5_webbridge_multi.py "${BASE_FLAGS[@]}" \
    --no_v54_psc2_use_gnn \
    --output outputs/omniview_fusion_v54_psc_v2_abl_no_gnn_local_4090.pth \
    >> outputs/omniview_fusion_v54_psc_v2_abl_no_gnn_local_4090.log 2>&1

echo "v54 ablation matrix complete."
