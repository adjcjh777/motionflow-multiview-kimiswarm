#!/usr/bin/env bash
# Calibration perturbation curriculum on the best cross-view PP model.
# Runs on the local WSL RTX 4090. Adjust batch_size if OOM.
set -e
cd "$(dirname "$0")/.."
...
python experiments/train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py \
    ...
    --cam_aug_schedule extrinsic_curriculum --cam_aug_ramp_epochs 10 \
