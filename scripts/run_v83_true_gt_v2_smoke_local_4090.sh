#!/usr/bin/env bash
# Local RTX 4090 smoke for v83 view-conditioned temporal attention on the
# NON-CIRCULAR H36M true-GT v2 standard protocol.
#
# v25 geometry fusion + view-conditioned temporal attention over ray tokens.
# Tiny schedule finishes in <= 30 min on the local RTX 4090.
#
# Run only when the local GPU is free (project rule: at most ONE training task
# at a time on the RTX 4090).
set -euo pipefail
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --output outputs/omniview_fusion_v83_true_gt_v2_h36m_smoke_local_4090.pth \
    > outputs/omniview_fusion_v83_true_gt_v2_h36m_smoke_local_4090.log 2>&1
