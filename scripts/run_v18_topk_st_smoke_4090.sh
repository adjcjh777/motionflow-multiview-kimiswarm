#!/usr/bin/env bash
# v18 deformable attention with straight-through top-k sampling smoke test.
# Tests whether the corrected sparse sampler can train stably on the local RTX 4090.
# Defaults are intentionally tiny so the smoke finishes in a few minutes.
set -euo pipefail

CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
PYTHON=${PYTHON:-python}
OUTPUT=${OUTPUT:-outputs/omniview_fusion_v18_topk_st_smoke_local_4090.pth}
LOG=${LOG:-outputs/omniview_fusion_v18_topk_st_smoke_local_4090.log}
EPOCHS=${EPOCHS:-2}
