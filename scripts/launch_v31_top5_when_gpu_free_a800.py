#!/usr/bin/env python3
"""Poll A800-D and launch the v31 top-5 smokes when a GPU frees.

NOTE: This script only *launches* on A800-D.  The remote repository must already
contain the v31 code (geometry-biased encoder, camera embedding, collision
penalty, skeleton residual gate); otherwise the flags will be unknown.  The
local repo pushes the code, but A800-D must be updated manually before running
this launcher.
"""
...
SSH_HOST = "a800-D"
MIN_FREE_MIB = 30000  # d=64 smoke on A800
...
RUNS = [
    ("v31_domain_balanced", "--use_hierarchical_multiview_v30", "omniview_fusion_v31_domain_balanced_a800"),
    ("v31_physical_floor_only", "--use_hierarchical_multiview_v30 --use_physical_space_temporal_loss_v29 --v29_floor_loss_weight 0.01 --v29_bone_temporal_weight 0.0 --v29_com_jitter_weight 0.0 --v29_physical_loss_warmup_epochs 3", "omniview_fusion_v31_physical_floor_only_a800"),
    ("v31_hierarchical_more_dropout", "--use_hierarchical_multiview_v30 --v30_dropout 0.3 --v30_stochastic_depth_prob 0.2", "omniview_fusion_v31_hierarchical_more_dropout_a800"),
    ("v31_outlier_view_adaptive", "--use_hierarchical_multiview_v30 --v25_use_outlier_view_detector --v25_outlier_z_thresh 2.0 --v25_outlier_soft_beta 2.0", "omniview_fusion_v31_outlier_view_adaptive_a800"),
    ("v31_geometry_attention", "--use_hierarchical_multiview_v31 --v31_geometry_bias", "omniview_fusion_v31_geometry_attention_a800"),
]
...
def launch_run(name: str, extra_flags: str, output: str, gpu: int) -> None:
    ...
    a800_ssh(f"tmux has-session -t {session} 2>/dev/null || tmux new-session -d -s {session} -n v31 '{cmd}'")
