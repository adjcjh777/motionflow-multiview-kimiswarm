#!/usr/bin/env bash
# Local RTX 4090 Shelf/Campus non-circular smoke for v25 geometry fusion.
# Uses only Shelf/Campus (true 3D GT, 5 views, 17 joints) to avoid the
# mixed-view epipolar bias crash seen with MPI-INF-3DHP (14 views).
set -euo pipefail

PYTHON=${PYTHON:-python}
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    --use_mixed_loader \
    --mixed_manifest configs/splits/shelf_campus_noncircular_smoke.yaml \
    ...
    --use_multiscale_fusion true --use_camera_conditioning true --use_epipolar_bias false \
    ...
    --use_entropy_regularization true --attention_entropy_weight 0.01 \
     ...
     --variable_view_min_views 2 --variable_view_max_views 5 \
    ...
    --output outputs/omniview_fusion_v25_shelf_campus_noncircular_smoke_debug.pth \
    > outputs/omniview_fusion_v25_shelf_campus_noncircular_smoke_debug.log 2>&1
