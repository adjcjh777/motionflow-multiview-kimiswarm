#!/usr/bin/env bash
# Small focal-aware intrinsic correction on WSL + RTX 4090.
set -euo pipefail
cd "$(dirname "$0")/.."
...
python -u experiments/train_ray_attention_temporal_residual_principal_point_mpiinf3dhp.py \
  ...
  --train_samples 500 --val_stride 50 --batch_size 8 --pp_loss_weight 0.1 --focal_max_scale 0.1 \
  --cam_aug_rot 0.5 --cam_aug_trans 0.005 --cam_aug_focal 0.01 --cam_aug_pp 5.0 \
  --output outputs/principal_point_pp_supervised_focal_small.pth \
  "$@"
