#!/usr/bin/env bash
# Run MVPose baseline on the corrected H36M true-GT protocol.
#
# This is a thin wrapper around
# scripts/sota_baselines/prepare_mvpose_h36m.sh. It performs the safety
# checks, creates the log directory, and redirects output to a log file so
# the run can be launched with nohup.
#
...
LOG="outputs/sota_baselines/mvpose_h36m_true_gt_run.log"
echo "Starting MVPose H36M true-GT baseline. Log: ${LOG}"

bash "${SCRIPT_DIR}/sota_baselines/prepare_mvpose_h36m.sh" > "${LOG}" 2>&1

echo "MVPose H36M true-GT baseline complete. See ${LOG}"
