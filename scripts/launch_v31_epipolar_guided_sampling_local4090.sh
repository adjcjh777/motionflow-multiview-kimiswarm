#!/usr/bin/env bash
# v31 epipolar-guided sampling smoke test on local RTX 4090.
#
# This run builds on the v30 hardened hierarchical encoder and adds the v18
# deformable cross-view attention with straight-through top-k sampling.  The
# top-k selection is driven by epipolar-geometry-biased attention scores, giving
# a concrete smoke test of epipolar-guided view sampling.
#
# TTE is intentionally left off (TTE is broken in v29/v30).  Physical loss is
# enabled with the same light weights used by the v30 smoke run.
#
# Usage:
#   bash scripts/launch_v31_epipolar_guided_sampling_local4090.sh
#
# Monitor:
#   tail -f outputs/omniview_fusion_v31_epipolar_guided_sampling_local_4090.log
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --deformable_attention_use_topk_st \
    ...
    --output outputs/omniview_fusion_v31_epipolar_guided_sampling_local_4090.pth \
    > outputs/omniview_fusion_v31_epipolar_guided_sampling_local_4090.log 2>&1 &

echo $! > outputs/v31_epipolar_guided_sampling_local_4090.pid
wait $!
