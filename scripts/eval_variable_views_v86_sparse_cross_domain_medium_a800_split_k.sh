#!/usr/bin/env bash
# Split v86 unified sparse-cross-domain variable-view eval into per-k chunks.
# Each chunk runs only one view count, so each finishes much faster than the
# full 2/3/4 sweep and survives environments where long processes are terminated.
set -euo pipefail
...
OUT_PREFIX="outputs/variable_view_v86_sparse_cross_domain_medium_a800"
...
for K in 2 3 4; do
    echo "[$(date -Iseconds)] Starting v86 sparse-cross-domain eval for k=${K}"
...
echo "v86 A800 sparse-cross-domain split-k no-fallback eval complete. Results: ${OUT_PREFIX}_k{2,3,4}.{csv,json}"
