#!/usr/bin/env bash
# Local RTX 4090 smoke test for v57 Domain-Conditional Physical-Space
# Calibration (DC-PSC) using only non-circular labels (MPI-INF-3DHP + Shelf/Campus).
# H36M is excluded because its 3D labels are circular (DLT triangulation of 2D).
#
...
python -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --output outputs/omniview_fusion_v57_mpi_only_noncircular_medium.pth \
    > outputs/omniview_fusion_v57_mpi_only_noncircular_medium.log 2>&1
