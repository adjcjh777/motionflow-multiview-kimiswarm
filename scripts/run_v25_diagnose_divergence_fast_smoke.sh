#!/usr/bin/env bash
# Fast local smoke to isolate v25 H36M true-GT divergence (<= 30 min).
#
# Same conservative schedule as the full smoke, but with far fewer samples per
# epoch so we can see whether validation MPJPE explodes after epoch 1/2 in a
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --epochs 3 --batch_size 16 --train_samples 256 --val_stride 20 \
    --lr 1e-4 --lr_cosine --lr_warmup_epochs 2 --lr_min 1e-6 \
    ...
    --output outputs/ablations/v25_true_gt_lowlr_no_permute_fast_smoke.pth \
    > outputs/ablations/v25_true_gt_lowlr_no_permute_fast_smoke.log 2>&1
