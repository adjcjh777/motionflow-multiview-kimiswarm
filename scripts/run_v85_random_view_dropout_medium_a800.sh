#!/usr/bin/env bash
# A800-D medium run for v85 random view dropout on the non-circular H36M
# true-GT standard protocol (S1,5,6,7,8 train -> S9/S11 val).
#
# v85 randomly drops entire camera views during training and injects an
# active-view-count embedding into the v25 ray tokens.  The goal is to
# improve sparse-view (k=2/k=3) robustness without hurting full-view k=4
# performance.
#
# Usage
# -----
#   # Default: GPU 7 (project only uses GPUs 6/7 on A800)
...
# GPU discipline: default to GPU 7 (project only uses GPUs 6/7), allow CUDA_VISIBLE_DEVICES override.
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --output outputs/ablations/v85_random_view_dropout_medium_a800.pth \
    > outputs/ablations/v85_random_view_dropout_medium_a800.log 2>&1
