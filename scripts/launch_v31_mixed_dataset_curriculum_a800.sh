#!/usr/bin/env bash
# v31 mixed-dataset curriculum full run on A800-D.
# Three-stage warm-start: H36M-only -> 3:1 H36M:MPI -> 1:1 H36M:MPI.
# NOTE: This script is a ready-to-launch template; do not run on A800-D
# unless a GPU is free and the launch has been approved by the team.
set -euo pipefail
...
GPU=${1:-0}
OUT_DIR=outputs/v31_mixed_dataset_curriculum_a800
...
COMMON_FLAGS=(
    ...
    --num_workers 4 --d 128 --residual_hidden 256 --n_st_layers 2
    --graph_num_layers 1 --n_joint_layers 1 --n_heads 4
    --clip_len 13 --batch_size 8 --train_samples 1000 --val_stride 10
...
echo "[v31 curriculum] Full run complete. Final checkpoint: ${STAGE3_PTH}"
