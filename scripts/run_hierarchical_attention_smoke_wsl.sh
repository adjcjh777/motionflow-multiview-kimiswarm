#!/usr/bin/env bash
# CPU smoke test for the hierarchical view -> temporal -> skeleton-joint attention model.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# Force CPU-only execution so the RTX 4090 remains free for the full Bayesian tri run.
...
python -u experiments/train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py \
  ...
  --model_type hierarchical_view_temporal_joint_pp \
  --pp_loss_weight 0.1 --cam_aug_pp 3.0 --cam_aug_focal 0.01 \
  --output outputs/hierarchical_attention_smoke.pth \
  > outputs/hierarchical_attention_smoke.log 2>&1
