#!/usr/bin/env bash
# A800-D medium run for v86: dedicated separate sparse-view head for k<4.
#
# Builds on the v85 recipe (random whole-view dropout + active-view-count
# embedding) and adds a small dedicated head that is applied whenever the
# number of active views is less than 4.  The head pools active-view ray tokens,
# injects a view-count embedding, and predicts a residual around the initial
# DLT estimate.  It is identity-at-init, so the full-view path is unchanged at
...
# Usage
# -----
#   # Default: GPU 6 (project only uses GPUs 6/7; override if busy)
#   bash scripts/run_v86_separate_sparse_view_head_medium_a800.sh
#
#   # Run on a specific free GPU
#   CUDA_VISIBLE_DEVICES=7 bash scripts/run_v86_separate_sparse_view_head_medium_a800.sh
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --output outputs/ablations/v86_separate_sparse_view_head_medium_a800.pth \
    > outputs/ablations/v86_separate_sparse_view_head_medium_a800.log 2>&1
