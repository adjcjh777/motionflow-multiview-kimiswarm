#!/usr/bin/env bash
# A800-D medium run for v87: sparse-view residual head on the corrected
# (non-circular) H36M true-GT v2 protocol (S1,5,6,7,8 train -> S9/S11 val).
#
# v87 inserts a count-conditioned residual MLP after the triangulation path inside
# MultiViewGeometryFusionV25.  The head is identity-at-init, so the k=4 path is
# not perturbed at start, while variable-view training exposes the model to k=2/3.
#
# Usage
# -----
#   # Default: run on GPU 6 (project only uses GPUs 6/7 on A800)
#   bash scripts/run_v87_true_gt_v2_medium_a800.sh
#
#   # Run on a specific free GPU
#   CUDA_VISIBLE_DEVICES=7 bash scripts/run_v87_true_gt_v2_medium_a800.sh
#
#   # Typical detached launch
#   nohup bash scripts/run_v87_true_gt_v2_medium_a800.sh \
#       > outputs/ablations/v87_true_gt_v2_medium_a800_nohup.log 2>&1 &
...
$PYTHON -u experiments/train_omniview_fusion_v5_webbridge_multi.py \
    ...
    --use_v87_sparse_view_residual_head \
    --v87_ssvr_head_hidden 128 \
    --v87_ssvr_head_n_layers 2 \
    --v87_ssvr_head_dropout 0.1 \
    --v87_ssvr_head_use_count_embedding \
    --num_workers 4 \
    --d 128 \
    --residual_hidden 256 \
    --n_st_layers 3 \
    ...
    --output outputs/ablations/v87_true_gt_v2_medium_a800.pth \
    > outputs/ablations/v87_true_gt_v2_medium_a800.log 2>&1
