#!/usr/bin/env bash
# Launch wrapper for the v81 true-GT v2 medium A800 run.
#
# Waits until GPU 6 or GPU 7 is free (project GPUs only; never touches 0-5),
# then launches scripts/run_v81_true_gt_v2_medium_a800.sh on the first
# available GPU in a persistent tmux session.
#
# Usage
# -----
#   # Foreground (blocks until a GPU is free and the job is launched)
#   bash scripts/launch_v81_true_gt_v2_medium_a800.sh
#
#   # Detached watcher
#   nohup bash scripts/launch_v81_true_gt_v2_medium_a800.sh \
#       > outputs/launch_v81_true_gt_v2_medium_a800.log 2>&1 &
#
...
log "Launch watcher started for v81 true-GT v2 medium run on ${A800_HOST}."
