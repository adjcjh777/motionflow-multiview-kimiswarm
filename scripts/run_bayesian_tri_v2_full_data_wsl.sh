#!/usr/bin/env bash
# Full-data large-scale MPI-INF-3DHP training for Bayesian triangulation v2.
# Uses all available train subjects (S1, S3, S4-S8) and the same stabilization
# settings as run_bayesian_tri_v2_stabilized_wsl.sh.  S2/Seq1 is kept for
# validation so that numbers remain comparable with previous runs.
set -euo pipefail
cd "$(dirname "$0")/.."
...
python -u experiments/train_ray_attention_temporal_crossview_residual_principal_point_mpiinf3dhp.py \
  ...
  --output outputs/bayesian_tri_v2_full_data_mpiinf3dhp.pth
